...
live VGPR set approaches the architectural limit. It is intentionally excluded
from the ordinary corpus pytest matrix: performance measurements require an
explicitly selected simulator configuration and controlled process-level
sampling. The compiler resource-profile verifier prevents compiler changes
from silently invalidating the experiment.
...
Representative corpus workloads were run five times per build. The table
reports process medians; each workload produced its expected correctness
result.

| Workload                         | Register pressure        | Baseline runtime | Lazy runtime | Baseline peak RSS | Lazy peak RSS |
|----------------------------------|--------------------------|-----------------:|-------------:|------------------:|--------------:|
| Corpus matvec set                | 6–88 VGPR                |           3.26 s |       1.86 s |    ~1,616,700 KiB |   305,864 KiB |
| Corpus HipKittens 256³           | 248 VGPR                 |           2.03 s |       0.60 s |    ~1,626,112 KiB |   314,368 KiB |
| Corpus hip-matmul 128³           | Up to 133 VGPR plus AGPR |           6.34 s |       4.91 s |    ~1,615,000 KiB |   302,940 KiB |

The IREE `softmax_static_10x256x256xf32` workload processes 655,360 FP32
elements and writes a 2.5 MiB output. It was compiled independently for each
target, run through IREE's HIP runtime under rocjitsu, and compared bit for bit
with the checked-in NumPy golden output. Five executions per build and target
were checked; the process measurements below are means of three executions.

| Target | Build          | Mean peak RSS | Mean runtime | Exact outputs |
|--------|----------------|--------------:|-------------:|--------------:|
| gfx942 | Baseline       | 1,627,477 KiB |       2.09 s |         5 / 5 |
| gfx942 | Lazy registers |   316,328 KiB |       0.70 s |         5 / 5 |
| gfx950 | Baseline       | 1,387,861 KiB |       1.84 s |         5 / 5 |
| gfx950 | Lazy registers |   338,881 KiB |       0.72 s |         5 / 5 |

The IREE executions were not CPU-pinned, so their runtime values are secondary
observations. Exact outputs and the stable cross-architecture RSS reduction
...
nonresident. Reaching the absolute 512-register limit would require a
...
The kernel corpus exposes the benchmark only when explicitly enabled. Configure
it with the same ROCm SDK and target used for rocjitsu, then let the default
build compile the selected workload:

```bash
cmake -S /path/to/rocjitsu-test-corpus/corpus/kernels \
  -B /path/to/build-vgpr-saturation -G Ninja \
  -DCMAKE_BUILD_TYPE=Release \
  -DCMAKE_HIP_ARCHITECTURES=gfx942 \
  -DROCM_PATH=/path/to/rocm \
  -DKERNEL_CORPUS_ENABLE_ALL=OFF \
  -DKERNEL_CORPUS_ENABLE_VGPR_SATURATION=ON
cmake --build /path/to/build-vgpr-saturation
```
...
The opt-in CMake configuration also exposes
`rocjitsu_vgpr_saturation_check`, which builds the executable and runs the same
resource verification:

```bash
cmake --build /path/to/build-vgpr-saturation \
  --target rocjitsu_vgpr_saturation_check
```
...
/usr/bin/time -v /path/to/rocjitsu \
  ...
  /path/to/build-vgpr-saturation/benchmarks/vgpr_saturation/rocjitsu_vgpr_saturation \
  --csv
```
...
./rocjitsu_vgpr_saturation --counts 8,64,128,192,252 \
...
GPU_MAX_HW_QUEUES=8 ./rocjitsu_vgpr_saturation --counts 252 --iterations 8 \
  --waves 1280 --streams 8 --samples 3 --warmups 0
```

Configured runtime warmups occur before every point, but rocjitsu's
demand-paged VGPR storage requests that a wave's physical pages be discarded
when that wave retires and falls back to zeroing if reclamation fails. Page
reclamation succeeded on the measurement host, so every timed launch includes
the page first-touch cost for its live register set.
