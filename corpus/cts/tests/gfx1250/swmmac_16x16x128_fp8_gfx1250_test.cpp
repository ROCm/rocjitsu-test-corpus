    ...
    for(int i = 0; i < M; ++i)
        for(int j = 0; j < N; ++j)
        {
            ...
            for(int cc = 0; cc < KC; ++cc)
            {
                ...
                acc         = acc
                      + fpsan::cast<CE>(VA(m.A[i * KC + cc])) * fpsan::cast<CE>(VB(m.B[k * N + j]));
            }
...
#define SWMMAC_TRAITS(NAME, AVEC, BVEC, CVEC, WRAP)                       \
    ...
    {                                                                     \
        ...
        __device__ static Value<CVec, S, C> call(Value<AVec, S, C>   a,   \
                                                 Value<BVec, S, C>   b,   \
                                                 Value<CVec, S, C>   c,   \
                                                 fpsan::v2i32_native idx) \
        {                                                                 \
