...
// after the move, each lane's payload (FPSan-family semantics) and float bits (Native
...
static bool ds_permute_uses_full_wave64() { return device_is_rdna4() || device_is_gfx950(); }
#endif

// ---- readlane ---------------------------------------------------------------
template <Semantics S, class Out> __global__ void k_readlane(Out *out, int from) {
...
template <Semantics S> void test_readlane(int from) {
  int ndev = 0;
...
TEST(Xlane, ReadlaneFloat17) { test_readlane<Semantics::Native>(17); }
TEST(Xlane, ReadlaneFpsan0) { FPSAN_RUN_FPSAN_SEMANTICS(test_readlane, 0); }
TEST(Xlane, ReadlaneFpsan17) { FPSAN_RUN_FPSAN_SEMANTICS(test_readlane, 17); }
#if FPSAN_TEST_FORCE_WAVE_SIZE == 64
TEST(Xlane, ReadlaneFloat48) { test_readlane<Semantics::Native>(48); }
TEST(Xlane, ReadlaneFpsan48) { FPSAN_RUN_FPSAN_SEMANTICS(test_readlane, 48); }
#endif

// ---- readfirstlane (= readlane(0) when lane 0 is active) --------------------
template <Semantics S, class Out> __global__ void k_readfirstlane(Out *out) {
  const int lane = threadIdx.x;
  Value<float, S, kCC> v{lane_input_float(lane)};
...
template <Semantics S> void test_readfirstlane() {
  int ndev = 0;
  if (hipGetDeviceCount(&ndev) != hipSuccess || ndev == 0)
    GTEST_SKIP() << "no HIP device";
  using Out = std::conditional_t<S == Semantics::Native, float, std::uint32_t>;
...
TEST(Xlane, ReadfirstlaneFpsan) { FPSAN_RUN_FPSAN_SEMANTICS(test_readfirstlane, ); }
...
template <Semantics S, class Out> __global__ void k_ds_bpermute_xor(Out *out, int off) {
  const int lane = threadIdx.x;
  Value<float, S, kCC> v{lane_input_float(lane)};
...
template <Semantics S> void test_ds_bpermute_xor(int off) {
  int ndev = 0;
  if (hipGetDeviceCount(&ndev) != hipSuccess || ndev == 0)
    GTEST_SKIP() << "no HIP device";
  using Out = std::conditional_t<S == Semantics::Native, float, std::uint32_t>;
...
TEST(Xlane, DsBpermuteXorFloat16) { test_ds_bpermute_xor<Semantics::Native>(16); }
TEST(Xlane, DsBpermuteXorFpsan1) { FPSAN_RUN_FPSAN_SEMANTICS(test_ds_bpermute_xor, 1); }
TEST(Xlane, DsBpermuteXorFpsan16) { FPSAN_RUN_FPSAN_SEMANTICS(test_ds_bpermute_xor, 16); }
#if FPSAN_TEST_FORCE_WAVE_SIZE == 64
// off=31 covers the largest same-half XOR in W64 mode.
TEST(Xlane, DsBpermuteXorFloat31) { test_ds_bpermute_xor<Semantics::Native>(31); }
TEST(Xlane, DsBpermuteXorFpsan31) { FPSAN_RUN_FPSAN_SEMANTICS(test_ds_bpermute_xor, 31); }
// off=32 is the cross-half boundary: gfx11 expects modulo-32 lane selection,
// while RDNA4/gfx950 expect a full-wave exchange.
TEST(Xlane, DsBpermuteXorFloat32Boundary) { test_ds_bpermute_xor<Semantics::Native>(32); }
TEST(Xlane, DsBpermuteXorFpsan32Boundary) { FPSAN_RUN_FPSAN_SEMANTICS(test_ds_bpermute_xor, 32); }
#endif
...
template <Semantics S, class Out> __global__ void k_ds_permute_xor(Out *out, int off) {
  const int lane = threadIdx.x;
  Value<float, S, kCC> v{lane_input_float(lane)};
...
template <Semantics S> void test_ds_permute_xor(int off) {
  int ndev = 0;
  if (hipGetDeviceCount(&ndev) != hipSuccess || ndev == 0)
    GTEST_SKIP() << "no HIP device";
  using Out = std::conditional_t<S == Semantics::Native, float, std::uint32_t>;
...
TEST(Xlane, DsPermuteXorFloat16) { test_ds_permute_xor<Semantics::Native>(16); }
TEST(Xlane, DsPermuteXorFpsan1) { FPSAN_RUN_FPSAN_SEMANTICS(test_ds_permute_xor, 1); }
TEST(Xlane, DsPermuteXorFpsan16) { FPSAN_RUN_FPSAN_SEMANTICS(test_ds_permute_xor, 16); }
#if FPSAN_TEST_FORCE_WAVE_SIZE == 64
// off=31 covers the largest same-half XOR in W64 mode.
TEST(Xlane, DsPermuteXorFloat31) { test_ds_permute_xor<Semantics::Native>(31); }
TEST(Xlane, DsPermuteXorFpsan31) { FPSAN_RUN_FPSAN_SEMANTICS(test_ds_permute_xor, 31); }
// off=32 is the cross-half boundary: gfx11 expects modulo-32 lane selection,
// while RDNA4/gfx950 expect a full-wave exchange.
TEST(Xlane, DsPermuteXorFloat32Boundary) { test_ds_permute_xor<Semantics::Native>(32); }
TEST(Xlane, DsPermuteXorFpsan32Boundary) { FPSAN_RUN_FPSAN_SEMANTICS(test_ds_permute_xor, 32); }
#endif
...
template <Semantics S, class Out> __global__ void k_ds_swizzle(Out *out, int pattern_select) {
  const int lane = threadIdx.x;
  Value<float, S, kCC> v{lane_input_float(lane)};
...
template <Semantics S> void test_ds_swizzle_lane_mapping() {
  if (!have_device())
    GTEST_SKIP() << "no HIP device";
  for (int sel = 0; sel < 2; ++sel) {
    ...
    for (int i = 0; i < LANES; ++i) {
      ...
      ASSERT_NE(src_lane, -1) << "Float output at lane " << i << " sel=" << sel
                              << " matches no source lane";
      const float src = static_cast<float>(src_lane * 7 + 1) - 100.f;
...
template <unsigned PAT, Semantics S, class Out> __global__ void k_ds_swizzle_pat(Out *out) {
  const int lane = threadIdx.x;
  Value<float, S, kCC> v{lane_input_float(lane)};
...
template <unsigned PAT> static void check_ds_swizzle_oracle(const char *tag) {
  using FOut = float;
  ...
  fpsan_test::for_each_fpsan_semantics([&](auto sem) {
    ...
    k_ds_swizzle_pat<PAT, S><<<1, LANES>>>(d_p);
    HIP_CHECK(hipDeviceSynchronize());
...
template <Semantics S, class Out> __global__ void k_mov_dpp_identity(Out *out) {
  const int lane = threadIdx.x;
  Value<float, S, kCC> v{lane_input_float(lane)};
...
template <Semantics S> void test_mov_dpp_identity() {
  int ndev = 0;
  if (hipGetDeviceCount(&ndev) != hipSuccess || ndev == 0)
    GTEST_SKIP() << "no HIP device";
  using Out = std::conditional_t<S == Semantics::Native, float, std::uint32_t>;
...
TEST(Xlane, MovDppIdentityFpsan) { FPSAN_RUN_FPSAN_SEMANTICS(test_mov_dpp_identity, ); }
...
template <Semantics S, class Out> __global__ void k_update_dpp_quad_swap_row_mask(Out *out) {
  const int lane = threadIdx.x;
...
template <Semantics S> void test_update_dpp_quad_swap_row_mask() {
  int ndev = 0;
  if (hipGetDeviceCount(&ndev) != hipSuccess || ndev == 0)
    GTEST_SKIP() << "no HIP device";
  using Out = std::conditional_t<S == Semantics::Native, float, std::uint32_t>;
...
template <Semantics S, class Out> __global__ void k_mov_dpp8_identity(Out *out) {
  const int lane = threadIdx.x;
  Value<float, S, kCC> v{lane_input_float(lane)};
...
template <Semantics S> void test_mov_dpp8_identity() {
  int ndev = 0;
  if (hipGetDeviceCount(&ndev) != hipSuccess || ndev == 0)
    GTEST_SKIP() << "no HIP device";
  using Out = std::conditional_t<S == Semantics::Native, float, std::uint32_t>;
...
template <Semantics S, class Out> __global__ void k_permlane64(const float *in, Out *out) {
  const int lane = threadIdx.x;
...
template <Semantics S> void test_permlane64() {
  int ndev = 0;
  if (hipGetDeviceCount(&ndev) != hipSuccess || ndev == 0)
    GTEST_SKIP() << "no HIP device";
  if (device_is_gfx950())
...
TEST(Xlane, Permlane64Fpsan) { FPSAN_RUN_FPSAN_SEMANTICS(test_permlane64, ); }
...
template <class FT> static __host__ __device__ inline FT wave_shfl_lane_input(int lane) {
...
template <class FT, Semantics S, bool HighHalfSource> void test_wave_shfl_quarter() {
  int ndev = 0;
  if (hipGetDeviceCount(&ndev) != hipSuccess || ndev == 0)
    GTEST_SKIP() << "no HIP device";
...
template <Semantics S> void test_wave_shfl_high_quarter_float() {
  test_wave_shfl_quarter<float, S, true>();
}

template <Semantics S> void test_wave_shfl_low_quarter_double() {
  test_wave_shfl_quarter<double, S, false>();
}

template <Semantics S> void test_wave_shfl_high_quarter_double() {
  test_wave_shfl_quarter<double, S, true>();
}

TEST(Xlane, WaveShflLowQuarterFloat) { test_wave_shfl_low_quarter_float<Semantics::Native>(); }

TEST(Xlane, WaveShflLowQuarterFpsan) {
  FPSAN_RUN_FPSAN_SEMANTICS(test_wave_shfl_low_quarter_float, );
}

TEST(Xlane, WaveShflHighQuarterFloat) { test_wave_shfl_high_quarter_float<Semantics::Native>(); }

TEST(Xlane, WaveShflHighQuarterFpsan) {
  FPSAN_RUN_FPSAN_SEMANTICS(test_wave_shfl_high_quarter_float, );
}

TEST(Xlane, WaveShflLowQuarterDouble) { test_wave_shfl_low_quarter_double<Semantics::Native>(); }

TEST(Xlane, WaveShflLowQuarterDoubleFpsan) {
  FPSAN_RUN_FPSAN_SEMANTICS(test_wave_shfl_low_quarter_double, );
}

TEST(Xlane, WaveShflHighQuarterDouble) { test_wave_shfl_high_quarter_double<Semantics::Native>(); }
...
enum class PermOp { Bcast, Down, Up, Xor };
...
template <PermOp Op> void test_permlane_family(int sel0, int sel1) {
  int ndev = 0;
  if (hipGetDeviceCount(&ndev) != hipSuccess || ndev == 0)
    GTEST_SKIP() << "no HIP device";
  if (!device_is_gfx1250())
  ...
  fpsan_test::for_each_fpsan_semantics([&](auto sem) {
    constexpr Semantics S = decltype(sem)::value;
    k_permlane_family<Op, S><<<1, LANES>>>(d_p, sel0, sel1);
    HIP_CHECK(hipDeviceSynchronize());
    HIP_CHECK(hipMemcpy(got_p.data(), d_p, LANES * sizeof(std::uint32_t), hipMemcpyDeviceToHost));
    using VP = Value<float, S, kCC>;
    for (int i = 0; i < LANES; ++i)
...
TEST(Xlane, PermlaneBcastCrossMode) { test_permlane_family<PermOp::Bcast>(0, 32); }
TEST(Xlane, PermlaneDownCrossMode) { test_permlane_family<PermOp::Down>(0x1, 32); }
TEST(Xlane, PermlaneUpCrossMode) { test_permlane_family<PermOp::Up>(0x1, 32); }
TEST(Xlane, PermlaneXorCrossMode) { test_permlane_family<PermOp::Xor>(0x1, 32); }
...
template <Semantics S, class Out> __global__ void k_ds_bpermute_fi_xor(Out *out, int off) {
  const int lane = threadIdx.x;
  Value<float, S, kCC> v{lane_input_float(lane)};
...
template <Semantics S> void test_ds_bpermute_fi_xor(int off) {
  int ndev = 0;
  if (hipGetDeviceCount(&ndev) != hipSuccess || ndev == 0)
    GTEST_SKIP() << "no HIP device";
  if (!device_is_gfx12())
...
TEST(Xlane, DsBpermuteFiXorFpsan1) { FPSAN_RUN_FPSAN_SEMANTICS(test_ds_bpermute_fi_xor, 1); }
#if FPSAN_TEST_FORCE_WAVE_SIZE == 64
TEST(Xlane, DsBpermuteFiXorFloat32) { test_ds_bpermute_fi_xor<Semantics::Native>(32); }
TEST(Xlane, DsBpermuteFiXorFpsan32) { FPSAN_RUN_FPSAN_SEMANTICS(test_ds_bpermute_fi_xor, 32); }
#endif
...
template <bool CROSS> static void check_permlane16(unsigned sel0, unsigned sel1, const char *tag) {
  float *d_f;
  std::uint32_t *d_p;
  HIP_CHECK(hipMalloc(&d_f, LANES * sizeof(float)));
  HIP_CHECK(hipMalloc(&d_p, LANES * sizeof(std::uint32_t)));
  ...
  fpsan_test::for_each_fpsan_semantics([&](auto sem) {
    constexpr Semantics S = decltype(sem)::value;
    k_permlane16<S, CROSS><<<1, LANES>>>(d_p, sel0, sel1);
    HIP_CHECK(hipDeviceSynchronize());
...
template <Semantics S, class Out> __global__ void k_permlane16_swap(Out *outx, Out *outy) {
  const int lane = threadIdx.x;
...
TEST(Xlane, Permlane16SwapCrossMode) {
  ...
  fpsan_test::for_each_fpsan_semantics([&](auto sem) {
    constexpr Semantics S = decltype(sem)::value;
    k_permlane16_swap<S><<<1, LANES>>>(dpx, dpy);
    HIP_CHECK(hipDeviceSynchronize());
    ...
    using VP = Value<float, S, kCC>;
    for (int i = 0; i < LANES; ++i) {
      EXPECT_EQ(px[i], VP{fx[i]}.fpsan_payload()) << fpsan::semantics_name(S) << " x lane " << i;
...
template <Semantics S, class Out> __global__ void k_permlane32_swap(Out *outx, Out *outy) {
  const int lane = threadIdx.x;
  Value<float, S, kCC> x{lane_input_float(lane)};
...
TEST(Xlane, Permlane32SwapCrossMode) {
  ...
  fpsan_test::for_each_fpsan_semantics([&](auto sem) {
    constexpr Semantics S = decltype(sem)::value;
    k_permlane32_swap<S><<<1, LANES>>>(dpx, dpy);
    HIP_CHECK(hipDeviceSynchronize());
    HIP_CHECK(hipMemcpy(px.data(), dpx, LANES * sizeof(std::uint32_t), hipMemcpyDeviceToHost));
    HIP_CHECK(hipMemcpy(py.data(), dpy, LANES * sizeof(std::uint32_t), hipMemcpyDeviceToHost));
    using VP = Value<float, S, kCC>;
