...
template <class FT> std::vector<FT> make_lane_inputs(int n) {
...
// ---------------------------------------------------------------------------
template <class T> void run_float_matches_host_butterfly() {
...
template <class T, Semantics S> void run_fpsan_matches_host_butterfly() {
  const int W = device_wave_size();
  if (W == 0)
    GTEST_SKIP() << "no HIP device";
  using FT = typename T::FT;
...
template <class T, Semantics S> void run_fpsan_strategy_invariant() {
  const int W = device_wave_size();
  if (W == 0)
    GTEST_SKIP() << "no HIP device";
  using FT = typename T::FT;
...
#define WAVE_TESTS(Trait)                                                                          \
  TEST(Trait, FloatMatchesHostButterfly) { run_float_matches_host_butterfly<Trait>(); }            \
