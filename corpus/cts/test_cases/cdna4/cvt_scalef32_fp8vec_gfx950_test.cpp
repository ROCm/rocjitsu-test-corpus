...
template <class FP8, class DstFT> struct UnpackVec;
template <> struct UnpackVec<fpsan::fp8_e4m3, float> {
...
template <> struct UnpackVec<fpsan::fp8_e5m2, float> {
  __device__ static float e0(int p, float s) {
...
template <> struct UnpackVec<fpsan::fp8_e4m3, _Float16> {
  __device__ static float e0(int p, float s) {
    return static_cast<float>(fpsan::amdgcn_cvt_scalef32_pk_f16_fp8<false, Semantics::Native, kCC>(
                                  p, Value<float, Semantics::Native, kCC>{s})
                                  .get(0)
                                  .to_float());
  }
};
template <> struct UnpackVec<fpsan::fp8_e5m2, _Float16> {
  __device__ static float e0(int p, float s) {
    return static_cast<float>(fpsan::amdgcn_cvt_scalef32_pk_f16_bf8<false, Semantics::Native, kCC>(
                                  p, Value<float, Semantics::Native, kCC>{s})
                                  .get(0)
                                  .to_float());
  }
};
template <> struct UnpackVec<fpsan::fp8_e4m3, __bf16> {
  __device__ static float e0(int p, float s) {
    return static_cast<float>(fpsan::amdgcn_cvt_scalef32_pk_bf16_fp8<false, Semantics::Native, kCC>(
                                  p, Value<float, Semantics::Native, kCC>{s})
                                  .get(0)
                                  .to_float());
  }
};
template <> struct UnpackVec<fpsan::fp8_e5m2, __bf16> {
  __device__ static float e0(int p, float s) {
    return static_cast<float>(fpsan::amdgcn_cvt_scalef32_pk_bf16_bf8<false, Semantics::Native, kCC>(
                                  p, Value<float, Semantics::Native, kCC>{s})
                                  .get(0)
                                  .to_float());
  }
...
template <class FP8, class DstFT> __global__ void k_unpack_all(const int *packed, float *out) {
  int l = threadIdx.x; // lane l carries fp8 byte l in word0/byte0.
  out[l] = UnpackVec<FP8, DstFT>::e0(packed[l], 1.0f);
}

template <class FP8, class DstFT> void run_unpack_all() {
  if (!have_device())
...
TEST(CvtScalef32Fp8Vec, UnpackAll_F32_Bf8) { run_unpack_all<fpsan::fp8_e5m2, float>(); }
TEST(CvtScalef32Fp8Vec, UnpackAll_F16_Fp8) { run_unpack_all<fpsan::fp8_e4m3, _Float16>(); }
TEST(CvtScalef32Fp8Vec, UnpackAll_F16_Bf8) { run_unpack_all<fpsan::fp8_e5m2, _Float16>(); }
TEST(CvtScalef32Fp8Vec, UnpackAll_BF16_Fp8) { run_unpack_all<fpsan::fp8_e4m3, __bf16>(); }
TEST(CvtScalef32Fp8Vec, UnpackAll_BF16_Bf8) { run_unpack_all<fpsan::fp8_e5m2, __bf16>(); }
...
template <class FP8, class SrcVEC> struct PackVec;
template <> struct PackVec<fpsan::fp8_e4m3, fpsan::v2h_native> {
...
template <> struct PackVec<fpsan::fp8_e4m3, fpsan::v2bf_native> {
  template <Semantics S>
  __device__ static int pack(int old, Value<fpsan::v2bf_native, S, kCC> v,
...
template <> struct PackVec<fpsan::fp8_e5m2, fpsan::v2h_native> {
  template <Semantics S>
  __device__ static int pack(int old, Value<fpsan::v2h_native, S, kCC> v, Value<float, S, kCC> sc) {
...
template <> struct PackVec<fpsan::fp8_e5m2, fpsan::v2bf_native> {
  template <Semantics S>
  __device__ static int pack(int old, Value<fpsan::v2bf_native, S, kCC> v,
                             Value<float, S, kCC> sc) {
...
template <class FP8, class SrcVEC, class SrcElem> void run_roundtrip() {
  if (!have_device())
    GTEST_SKIP() << "no HIP device";
