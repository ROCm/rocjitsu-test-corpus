...
template <class Traits> __device__ inline int gfx11_out_n(int lane) { return lane & 15; }

template <class Traits> __device__ inline int gfx11_out_m(int lane, int reg) {
  return (Traits::wave_size / 16) * reg + (lane >> 4);
}

template <class Traits> __device__ inline int gfx11_out_index(int reg) {
...
namespace {
template <class T> FPSAN_HOST_DEVICE T exact_int(int v) {
  return static_cast<T>(static_cast<float>(v));
}

template <class T> FPSAN_HOST_DEVICE T preserve_seed(int lane, int idx) {
...
template <class T> float layout_tolerance() {
