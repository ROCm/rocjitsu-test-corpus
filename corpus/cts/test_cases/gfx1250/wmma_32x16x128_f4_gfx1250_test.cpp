...
__global__ void
    k_f4(const std::uint32_t* Apack, const std::uint32_t* Bpack, const float* C, float* D)
{
...
TEST(WmmaF4_32x16x128, LayoutMatchesHardware)
{
    ...
    for(int m = 0; m < M; ++m)
        for(int k = 0; k < K; ++k)
            pack_nibble(
                &Apack[a_lane(m, k) * 16], a_slot(m, k), f32_to_narrow(A[m * K + k], kFp4E2M1));
    for(int n = 0; n < N; ++n)
        for(int k = 0; k < K; ++k)
    ...
    for(int i = 0; i < M * N; ++i)
        ...
        {
            if(mism < 8)
                ADD_FAILURE() << "mismatch at m=" << (i / N) << " n=" << (i % N)
                              << " got=" << got[i] << " ref=" << ref[i];
            ++mism;
        }
...
__global__ void
    k_wrap_float(const std::uint32_t* Apack, const std::uint32_t* Bpack, const float* C, float* D)
{
    const int     lane = threadIdx.x;
    v16i32_native a;
    v8i32_native  b;
...
template <int Cmod>
__global__ void
    k_f4_mod_float(const std::uint32_t* Apack, const std::uint32_t* Bpack, const float* C, float* D)
{
    const int     lane = threadIdx.x;
    v16i32_native a;
    v8i32_native  b;
...
TEST(WmmaF4_32x16x128, ModifierAbsC)
{
    run_modifier_f4<2>();
}
TEST(WmmaF4_32x16x128, ModifierNegAbsC)
{
    run_modifier_f4<3>();
}
...
static float e8m0f(int e)
{
    return std::ldexp(1.f, e - 127);
}

template <int SFMT>
__global__ void k_scale_float(const std::uint32_t* Apack,
                              const std::uint32_t* Bpack,
                              const float*         C,
                              const unsigned*      SA,
                              const unsigned*      SB,
...
template <int SFMT>
__global__ void k_scale16_float(const std::uint32_t*      Apack,
                                const std::uint32_t*      Bpack,
                                const float*              C,
...
{
    run_scale<false, 0>("scale");
}
TEST(WmmaF4_32x16x128, Scale16FloatAndFPSan)
{
    run_scale<true, 0>("scale16");
}
...
TEST(WmmaF4_32x16x128, ScaleE4M3FloatAndFPSan)
{
    run_scale<false, 2>("scale_e4m3");
}
TEST(WmmaF4_32x16x128, Scale16E4M3FloatAndFPSan)
{
    run_scale<true, 2>("scale16_e4m3");
}
...
TEST(WmmaF4_32x16x128, DISABLED_ProbeOneHotLane0)
{
    onehot_sweep(0);
}
TEST(WmmaF4_32x16x128, DISABLED_ProbeOneHotLane1)
{
    onehot_sweep(1);
}
TEST(WmmaF4_32x16x128, DISABLED_ProbeOneHotLane16)
{
    onehot_sweep(16);
}
...
TEST(WmmaF4_32x16x128, DISABLED_ProbeKrow0Lane0)
{
    kprobe_row0(0);
}
TEST(WmmaF4_32x16x128, DISABLED_ProbeKrow0Lane16)
{
    kprobe_row0(16);
}

TEST(WmmaF4_32x16x128, DISABLED_ProbeOneHotLane8)
{
    onehot_sweep(8);
}
TEST(WmmaF4_32x16x128, DISABLED_ProbeOneHotLane15)
{
    onehot_sweep(15);
}
...
__global__ void k_raw_scale(const std::uint32_t* Apack,
                            const std::uint32_t* Bpack,
                            const unsigned*      SA,
                            const unsigned*      SB,
                            float*               Draw)
{
    ...
    v16f_native d  = __builtin_amdgcn_wmma_scale_f32_32x16x128_f4(
        a, b, static_cast<short>(0), c, 0, 0, sa, 0, 0, sb, false, false);
    for(int r = 0; r < 16; ++r)
        Draw[lane * 16 + r] = d[r];
}
...
TEST(WmmaF4_32x16x128, DISABLED_ProbeScaleLane)
{
    scale_probe();
}
...
TEST(WmmaF4_32x16x128, DISABLED_ProbeScaleByteRow0)
{
    scale_byte_probe(0);
}
