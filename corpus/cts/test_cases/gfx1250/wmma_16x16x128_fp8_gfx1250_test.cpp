...
//
// tests/wmma_16x16x128_fp8_gfx1250_test.cpp
//
// Deep tests for the gfx1250 16x16x128 fp8/bf8 WMMA FPSan wrappers (K=128, 8-bit
// operands; f32 and f16 accumulators). Same two-property structure as the K=64
// fp8 tests: LayoutMatchesHardware (software dataflow vs real builtin, bit-exact
// on exact-integer inputs -> validates Wmma16x16x128Layout) and
// FpsanMatchesScalarReference (shipped FPSan path vs host scalar FPSan matmul).
//
...
// Inverse of Wmma16x16x128Layout: lane + v64 fragment slot idx -> logical K.
...
k_float_dataflow(const typename Harness<Traits>::AElem *A, const typename Harness<Traits>::BElem *B,
  ...
  Value<typename Harness<Traits>::CVec, Semantics::Native, kCC> c;
  load_frags<Traits, Semantics::Native>(A, B, C, lane, a, b, c);
  auto d = fpsan::detail::wmma_16x16x128_dataflow(a, b, c);
  for (int e = 0; e < 8; ++e)
    D[(e + 8 * (lane >> 4)) * N + (lane & 15)] = d.get(e).to_float();
}
...
// K=128: keep magnitudes small so 128-term integer sums stay exact in f32/f16
// (|sum| <= 128*4 + 4 = 516 < 2048).
#define FPSAN_WMMA128_TRAITS(NAME, AVEC, BVEC, CVEC, CALL)                                         \
  struct NAME {                                                                                    \
    using AVec = AVEC;                                                                             \
    using BVec = BVEC;                                                                             \
    using CVec = CVEC;                                                                             \
...
FPSAN_WMMA128_TRAITS(WmmaF32Fp8Fp8_128, v64e4m3_native, v64e4m3_native, v8f_native,
                     amdgcn_wmma_f32_16x16x128_fp8_fp8)
FPSAN_WMMA128_TRAITS(WmmaF32Fp8Bf8_128, v64e4m3_native, v64e5m2_native, v8f_native,
                     amdgcn_wmma_f32_16x16x128_fp8_bf8)
FPSAN_WMMA128_TRAITS(WmmaF32Bf8Fp8_128, v64e5m2_native, v64e4m3_native, v8f_native,
                     amdgcn_wmma_f32_16x16x128_bf8_fp8)
FPSAN_WMMA128_TRAITS(WmmaF32Bf8Bf8_128, v64e5m2_native, v64e5m2_native, v8f_native,
                     amdgcn_wmma_f32_16x16x128_bf8_bf8)
FPSAN_WMMA128_TRAITS(WmmaF16Fp8Fp8_128, v64e4m3_native, v64e4m3_native, v8h_native,
                     amdgcn_wmma_f16_16x16x128_fp8_fp8)
FPSAN_WMMA128_TRAITS(WmmaF16Fp8Bf8_128, v64e4m3_native, v64e5m2_native, v8h_native,
                     amdgcn_wmma_f16_16x16x128_fp8_bf8)
FPSAN_WMMA128_TRAITS(WmmaF16Bf8Fp8_128, v64e5m2_native, v64e4m3_native, v8h_native,
