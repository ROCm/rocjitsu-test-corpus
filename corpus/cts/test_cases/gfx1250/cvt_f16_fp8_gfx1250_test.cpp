...
struct DecodeOps;
template <>
struct DecodeOps<fpsan::fp8_e4m3>
{
    template <int B, Semantics S>
    __device__ static Value<_Float16, S, kCC> f16(int p)
    {
        return fpsan::amdgcn_cvt_f16_fp8<B, S, kCC>(p);
    }
};
template <>
struct DecodeOps<fpsan::fp8_e5m2>
{
    template <int B, Semantics S>
    __device__ static Value<_Float16, S, kCC> f16(int p)
    {
...
TEST(CvtF16Fp8, Fp8DecodeAllBytes)
{
    run_decode_all_bytes<fpsan::fp8_e4m3>();
}
TEST(CvtF16Fp8, Bf8DecodeAllBytes)
{
    run_decode_all_bytes<fpsan::fp8_e5m2>();
}
...
struct PkOps;
template <>
struct PkOps<fpsan::fp8_e4m3>
{
    template <Semantics S>
    __device__ static short pack(Value<fpsan::v2h_native, S, kCC> ab)
    {
        return fpsan::amdgcn_cvt_pk_fp8_f16<S, kCC>(ab);
    }
    template <Semantics S>
    __device__ static Value<fpsan::v2h_native, S, kCC> unpack(short p)
    {
        return fpsan::amdgcn_cvt_pk_f16_fp8<S, kCC>(p);
    }
};
template <>
struct PkOps<fpsan::fp8_e5m2>
{
    template <Semantics S>
    __device__ static short pack(Value<fpsan::v2h_native, S, kCC> ab)
    {
...
void run_pk_roundtrip()
{
    ...
    }
    {
        ...
        HIP_CHECK(
            hipMemcpy(got.data(), dO, 2 * LANES * sizeof(std::uint16_t), hipMemcpyDeviceToHost));
        for(int i = 0; i < 2 * LANES; ++i)
...
TEST(CvtF16Fp8, Fp8PkRoundTrip)
{
    run_pk_roundtrip<fpsan::fp8_e4m3>();
}
TEST(CvtF16Fp8, Bf8PkRoundTrip)
{
    run_pk_roundtrip<fpsan::fp8_e5m2>();
}
...
TEST(CvtF16Fp8, Fp8PackByteOrder)
{
    run_pack_bytes<fpsan::fp8_e4m3>();
}
TEST(CvtF16Fp8, Bf8PackByteOrder)
{
    run_pack_bytes<fpsan::fp8_e5m2>();
}
...
struct SrOps<fpsan::fp8_e4m3>
{
    ...
    template <Semantics S>
    __device__ static Value<_Float16, S, kCC> dec(int p)
    {
        return fpsan::amdgcn_cvt_f16_fp8<0, S, kCC>(p);
    }
};
template <>
struct SrOps<fpsan::fp8_e5m2>
{
    template <Semantics S>
    __device__ static int sr(Value<_Float16, S, kCC> v, int old, unsigned seed)
    {
...
TEST(CvtF16Fp8, Fp8SrRoundTrip)
{
    run_sr_roundtrip<fpsan::fp8_e4m3>();
}
TEST(CvtF16Fp8, Bf8SrRoundTrip)
{
    run_sr_roundtrip<fpsan::fp8_e5m2>();
}
