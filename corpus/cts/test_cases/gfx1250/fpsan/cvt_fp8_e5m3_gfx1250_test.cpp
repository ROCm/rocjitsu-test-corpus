...
template <int ByteIdx> __global__ void k_e5m3_decode_bytesel(const int *packed, float *out) {
  int l = threadIdx.x;
  out[l] = fpsan::amdgcn_cvt_f32_fp8_e5m3<ByteIdx, Semantics::Native, kCC>(packed[l]).to_float();
}

template <int ByteIdx> void run_decode_bytesel() {
  if (!have_device())
    GTEST_SKIP() << "no HIP device";
...
// ===========================================================================
template <bool DstLo> __global__ void k_e5m3_pack(int old, float a, float b, int *out) {
...
template <bool DstLo> void run_pack() {
  if (!have_device())
    GTEST_SKIP() << "no HIP device";
...
TEST(CvtFp8E5M3, PackHighHalf) { run_pack<false>(); }
...
// ===========================================================================
template <int ByteIdx> __global__ void k_e5m3_sr(int old, float v, unsigned seed, int *out) {
...
// the device builtin.
// ===========================================================================
template <Semantics S> __global__ void k_e5m3_fpsan_decode(const int *packed, unsigned *out) {
  int l = threadIdx.x;
...
TEST(CvtFp8E5M3, FpsanDecodeWiden8) {
  ...
  fpsan_test::for_each_fpsan_semantics([&](auto sem) {
    ...
    auto got = from_dev(dO, 256);
    for (int b = 0; b < 256; ++b) {
      auto want = fpsan::detail::packed_field_widen<8, S, kCC>(static_cast<std::uint32_t>(b))
                      .fpsan_payload();
