...
__device__ void load_frags(const float *A, const float *B, const float *C, int lane,
                           Value<v2f_native, S, kCC> &a, Value<v2f_native, S, kCC> &b,
...
template <Semantics S> void run_fpsan_matches_scalar_reference() {
  int ndev = 0;
  if (hipGetDeviceCount(&ndev) != hipSuccess || ndev == 0)
    GTEST_SKIP() << "no HIP device";
  Mats m = make_inputs();
