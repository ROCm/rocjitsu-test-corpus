...
template <Semantics S> __global__ void k_g_wrap_f16(const std::uint16_t *gmem, std::uint16_t *out) {
  using V = Value<_Float16, S, kCC>;
  int lane = threadIdx.x;
  auto r =
      fpsan::amdgcn_global_load_tr16_b128_f16<S, kCC>(reinterpret_cast<const V *>(&gmem[lane * 8]));
  for (int s = 0; s < 8; ++s)
...
template <Semantics S> __global__ void k_d_wrap_f16(const std::uint16_t *gmem, std::uint16_t *out) {
  using V = Value<_Float16, S, kCC>;
  __shared__ std::uint16_t lds[WAVE * 8];
  int lane = threadIdx.x;
  for (int s = 0; s < 8; ++s)
    lds[lane * 8 + s] = gmem[lane * 8 + s];
  __syncthreads();
  auto r = fpsan::amdgcn_ds_load_tr16_b128_f16<S, kCC>(reinterpret_cast<const V *>(&lds[lane * 8]));
  for (int s = 0; s < 8; ++s)
    out[lane * 8 + s] = r.get(s).to_storage_bits();
}
...
TEST(GlobalLoadTr16Gfx1250, F16_MatchesBuiltinAndFpsanMovesSameBits) {
  ...
  fpsan_test::for_each_fpsan_semantics([&](auto sem) {
    auto fps = run(k_g_wrap_f16<decltype(sem)::value>);
    for (size_t i = 0; i < raw.size(); ++i)
...
TEST(GlobalLoadTr16Gfx1250, BF16_MatchesBuiltinAndFpsanMovesSameBits) {
  ...
  for (size_t i = 0; i < raw.size(); ++i)
    EXPECT_EQ(flt[i], raw[i]) << "Float wrapper != builtin at " << i;
  fpsan_test::for_each_fpsan_semantics([&](auto sem) {
    auto fps = run(k_g_wrap_bf16<decltype(sem)::value>);
    for (size_t i = 0; i < raw.size(); ++i)
      EXPECT_EQ(fps[i], raw[i]) << "FPSan wrapper != builtin at " << i;
  });
}
...
template <Semantics S> __global__ void k_d_wrap_tr8(std::uint8_t *out) {
...
template <Semantics S> __global__ void k_g_wrap_tr8(const std::uint8_t *gmem, std::uint8_t *out) {
  using V = Value<fpsan::fp8_e4m3, S, kCC>;
  int lane = threadIdx.x;
  auto r =
      fpsan::amdgcn_global_load_tr8_b64_fp8<S, kCC>(reinterpret_cast<const V *>(&gmem[lane * 8]));
  for (int s = 0; s < 8; ++s)
    out[lane * 8 + s] = r.get(s).to_storage_bits();
}
...
template <Semantics S> __global__ void k_d_wrap_tr4(std::uint32_t *out) {
  __shared__ std::uint32_t lds[WAVE * 2];
  int lane = threadIdx.x;
  lds[lane * 2 + 0] = 0x12345670u + lane;
  lds[lane * 2 + 1] = 0x89abcdefu - lane;
...
template <Semantics S> __global__ void k_d_wrap_tr6(std::uint32_t *out) {
  __shared__ std::uint32_t lds[WAVE * 3];
  int lane = threadIdx.x;
  lds[lane * 3 + 0] = 0x0f1e2d3cu + lane;
  lds[lane * 3 + 1] = 0x4b5a6978u - lane;
...
TEST(DsLoadTr8Gfx1250, FP8_MatchesBuiltinAndFpsanMovesSameBits) {
  ...
  for (int i = 0; i < N; ++i)
    EXPECT_EQ(flt[i], raw[i]) << "Float wrapper != builtin at " << i;
  fpsan_test::for_each_fpsan_semantics([&](auto sem) {
    auto fps = run_self<std::uint8_t>(k_d_wrap_tr8<decltype(sem)::value>, N);
    for (int i = 0; i < N; ++i)
      EXPECT_EQ(fps[i], raw[i]) << "FPSan wrapper != builtin at " << i;
  });
}
...
TEST(GlobalLoadTr8Gfx1250, FP8_MatchesBuiltinAndFpsanMovesSameBits) {
  ...
  for (int i = 0; i < N; ++i)
    EXPECT_EQ(flt[i], raw[i]) << "Float wrapper != builtin at " << i;
  fpsan_test::for_each_fpsan_semantics([&](auto sem) {
    auto fps = run_g8(k_g_wrap_tr8<decltype(sem)::value>);
    for (int i = 0; i < N; ++i)
      EXPECT_EQ(fps[i], raw[i]) << "FPSan wrapper != builtin at " << i;
  });
}
...
TEST(DsLoadTr4Gfx1250, FP4_MatchesBuiltinAndFpsanMovesSameBits) {
  ...
  for (int i = 0; i < N; ++i)
    EXPECT_EQ(flt[i], raw[i]) << "Float wrapper != builtin at " << i;
  fpsan_test::for_each_fpsan_semantics([&](auto sem) {
    auto fps = run_self<std::uint32_t>(k_d_wrap_tr4<decltype(sem)::value>, N);
    for (int i = 0; i < N; ++i)
      EXPECT_EQ(fps[i], raw[i]) << "FPSan wrapper != builtin at " << i;
  });
}
...
TEST(DsLoadTr6Gfx1250, FP6_MatchesBuiltinAndFpsanMovesSameBits) {
  ...
  for (int i = 0; i < N; ++i)
    EXPECT_EQ(flt[i], raw[i]) << "Float wrapper != builtin at " << i;
  fpsan_test::for_each_fpsan_semantics([&](auto sem) {
    auto fps = run_self<std::uint32_t>(k_d_wrap_tr6<decltype(sem)::value>, N);
    for (int i = 0; i < N; ++i)
      EXPECT_EQ(fps[i], raw[i]) << "FPSan wrapper != builtin at " << i;
  });
}
...
TEST(DsLoadTr16Gfx1250, F16_MatchesBuiltinAndFpsanMovesSameBits) {
  ...
  for (size_t i = 0; i < raw.size(); ++i)
    EXPECT_EQ(flt[i], raw[i]) << "Float wrapper != builtin at " << i;
  fpsan_test::for_each_fpsan_semantics([&](auto sem) {
    auto fps = run(k_d_wrap_f16<decltype(sem)::value>);
    for (size_t i = 0; i < raw.size(); ++i)
      EXPECT_EQ(fps[i], raw[i]) << "FPSan wrapper != builtin at " << i;
  });
}
...
TEST(DsLoadTr16Gfx1250, BF16_MatchesBuiltinAndFpsanMovesSameBits) {
  ...
  for (size_t i = 0; i < raw.size(); ++i)
    EXPECT_EQ(flt[i], raw[i]) << "Float wrapper != builtin at " << i;
  fpsan_test::for_each_fpsan_semantics([&](auto sem) {
    auto fps = run(k_d_wrap_bf16<decltype(sem)::value>);
    for (size_t i = 0; i < raw.size(); ++i)
      EXPECT_EQ(fps[i], raw[i]) << "FPSan wrapper != builtin at " << i;
  });
}
