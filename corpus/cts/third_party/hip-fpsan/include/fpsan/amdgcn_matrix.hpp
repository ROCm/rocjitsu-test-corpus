...
// amdgcn_mfma.hpp).
template <class Elem, int N> struct vec_fragment {
  ...
  FPSAN_HOST_DEVICE vec_fragment(bits_t b) { *this = __builtin_bit_cast(vec_fragment, b); }
...
template <class Elem> using v8_fragment = vec_fragment<Elem, 8>;
...
template <class Elem> using v64_fragment = vec_fragment<Elem, 64>;
...
template <class Elem> struct v4_fragment {
  using bits_t = std::uint8_t __attribute__((ext_vector_type(4)));
  Elem e[4]{};
  FPSAN_HOST_DEVICE constexpr v4_fragment() = default;
  FPSAN_HOST_DEVICE v4_fragment(bits_t b) { *this = __builtin_bit_cast(v4_fragment, b); }
  FPSAN_HOST_DEVICE operator bits_t() const { return __builtin_bit_cast(bits_t, *this); }
  FPSAN_HOST_DEVICE constexpr Elem &operator[](unsigned i) { return e[i]; }
  FPSAN_HOST_DEVICE constexpr Elem operator[](unsigned i) const { return e[i]; }
...
FPSAN_DEVICE inline int wave_lane() { return __builtin_amdgcn_mbcnt_lo(~0u, 0u); }
...
// same-half DS gather by the low 5 bits of the absolute source lane.
  ...
  if (__builtin_amdgcn_wavefrontsize() == 64) {
    ...
    auto shuffle = static_cast<std::uint32_t>(__builtin_amdgcn_permlane64(static_cast<int>(word)));
...
struct Wmma16x16x16Layout {
  ...
  FPSAN_DEVICE static int ab_reg(int k) { return 2 * (k >> 3) + ((k >> 1) & 1); }
  FPSAN_DEVICE static int ab_half(int k) { return k & 1; }
  ...
  FPSAN_DEVICE static int cd_n(int lane) { return lane & 15; }
};
...
FPSAN_DEVICE Value<CVec, S, C> wmma_16x16x16_dataflow(Value<AVec, S, C> a, Value<BVec, S, C> b,
                                                      Value<CVec, S, C> c) {
...
struct WmmaGfx11_16x16x16Layout {
  FPSAN_DEVICE static int ab_lane(int row_or_col) { return row_or_col; }
  FPSAN_DEVICE static int ab_index(int k) { return k; }
  FPSAN_DEVICE static int cd_n(int lane) { return lane & 15; }
  FPSAN_DEVICE static int cd_m(int lane, int reg, int wave_size) {
    return (wave_size / 16) * reg + (lane >> 4);
  }
  FPSAN_DEVICE static int cd_half_index(int reg, bool opsel) { return 2 * reg + (opsel ? 1 : 0); }
};
...
wmma_gfx11_f32_16x16x16_dataflow(Value<AVec, S, C> a, Value<BVec, S, C> b, Value<CVec, S, C> c) {
  using DFrag = Value<CVec, S, C>;
...
template <bool Opsel, bool PreserveUnselected = false, class AVec, class BVec, class CVec,
          Semantics S, Conversions C>
FPSAN_DEVICE Value<CVec, S, C>
wmma_gfx11_half_16x16x16_dataflow(Value<AVec, S, C> a, Value<BVec, S, C> b, Value<CVec, S, C> c) {
  using DFrag = Value<CVec, S, C>;
  using AccScalar = typename DFrag::element_type;
  using Acc = Value<AccScalar, S, C>;
  static constexpr int E = static_cast<int>(DFrag::lanes);
...
struct Wmma16x16x4Layout {
  ...
  FPSAN_DEVICE static int ab_index(int k) { return k & 1; }
  FPSAN_DEVICE static int cd_m(int lane, int e) { return e + 8 * (lane >> 4); }
  FPSAN_DEVICE static int cd_n(int lane) { return lane & 15; }
};
...
template <class AVec, class BVec, class CVec, Semantics S, Conversions C>
FPSAN_DEVICE Value<CVec, S, C> wmma_16x16x4_dataflow(Value<AVec, S, C> a, Value<BVec, S, C> b,
                                                     Value<CVec, S, C> c) {
  using DFrag = Value<CVec, S, C>;
  using AccScalar = typename DFrag::element_type;
  using Acc = Value<AccScalar, S, C>;
...
template <int E, class AVec, class AccScalar, Semantics S, Conversions C>
FPSAN_DEVICE inline void wmma_16x16x32_accumulate_row(const Value<AVec, S, C> &a, int m_base,
                                                      int idx, int k, Value<AccScalar, S, C> bv,
                                                      Value<AccScalar, S, C> &acc) {
...
template <class AVec, class BVec, class CVec, Semantics S, Conversions C>
FPSAN_DEVICE Value<CVec, S, C> wmma_16x16x32_dataflow(Value<AVec, S, C> a, Value<BVec, S, C> b,
                                                      Value<CVec, S, C> c) {
  using DFrag = Value<CVec, S, C>;
  using AccScalar = typename DFrag::element_type;
  using Acc = Value<AccScalar, S, C>;
  ...
  for (int k = 0; k < 32; ++k) {
    ...
    const Acc bv = cast<AccScalar>(wave_shfl(b.get(idx), Wmma16x16x32Layout::ab_lane(n, k)));
    wmma_16x16x32_accumulate_row<0>(a, m_base, idx, k, bv, acc0);
    wmma_16x16x32_accumulate_row<1>(a, m_base, idx, k, bv, acc1);
    wmma_16x16x32_accumulate_row<2>(a, m_base, idx, k, bv, acc2);
    wmma_16x16x32_accumulate_row<3>(a, m_base, idx, k, bv, acc3);
    wmma_16x16x32_accumulate_row<4>(a, m_base, idx, k, bv, acc4);
    wmma_16x16x32_accumulate_row<5>(a, m_base, idx, k, bv, acc5);
    wmma_16x16x32_accumulate_row<6>(a, m_base, idx, k, bv, acc6);
    wmma_16x16x32_accumulate_row<7>(a, m_base, idx, k, bv, acc7);
  }
  DFrag d{};
  d.set(0, acc0);
  d.set(1, acc1);
...
FPSAN_DEVICE Value<DVec, S, C>
wmma_16x16x32_dataflow_cdiff(Value<AVec, S, C> a, Value<BVec, S, C> b, Value<CVec, S, C> c) {
  ...
  Value<DVec, S, C> d{};
  d.set(0, cast<DScalar>(acc0));
  d.set(1, cast<DScalar>(acc1));
  d.set(2, cast<DScalar>(acc2));
  d.set(3, cast<DScalar>(acc3));
  d.set(4, cast<DScalar>(acc4));
  d.set(5, cast<DScalar>(acc5));
  d.set(6, cast<DScalar>(acc6));
  d.set(7, cast<DScalar>(acc7));
  return d;
}
...
template <int E, class AVec, class AccScalar, Semantics S, Conversions C>
FPSAN_DEVICE inline void wmma_16x16x64_accumulate_row(const Value<AVec, S, C> &a, int m_base,
                                                      int idx, int k, Value<AccScalar, S, C> bv,
                                                      Value<AccScalar, S, C> &acc) {
  const int m = m_base + E;
...
template <class AVec, class BVec, class CVec, Semantics S, Conversions C>
FPSAN_DEVICE Value<CVec, S, C> wmma_16x16x64_dataflow(Value<AVec, S, C> a, Value<BVec, S, C> b,
                                                      Value<CVec, S, C> c) {
  using DFrag = Value<CVec, S, C>;
  using AccScalar = typename DFrag::element_type;
  using Acc = Value<AccScalar, S, C>;
  ...
  for (int k = 0; k < 64; ++k) {
    ...
    const Acc bv = cast<AccScalar>(wave_shfl(b.get(idx), Wmma16x16x64Layout::ab_lane(n, k)));
    wmma_16x16x64_accumulate_row<0>(a, m_base, idx, k, bv, acc0);
    wmma_16x16x64_accumulate_row<1>(a, m_base, idx, k, bv, acc1);
    wmma_16x16x64_accumulate_row<2>(a, m_base, idx, k, bv, acc2);
    wmma_16x16x64_accumulate_row<3>(a, m_base, idx, k, bv, acc3);
    wmma_16x16x64_accumulate_row<4>(a, m_base, idx, k, bv, acc4);
    wmma_16x16x64_accumulate_row<5>(a, m_base, idx, k, bv, acc5);
    wmma_16x16x64_accumulate_row<6>(a, m_base, idx, k, bv, acc6);
    wmma_16x16x64_accumulate_row<7>(a, m_base, idx, k, bv, acc7);
  }
  DFrag d{};
  d.set(0, acc0);
  d.set(1, acc1);
...
template <int E, class AVec, class AccScalar, Semantics S, Conversions C>
FPSAN_DEVICE inline void wmma_16x16x128_accumulate_row(const Value<AVec, S, C> &a, int m_base,
                                                       int idx, int k, Value<AccScalar, S, C> bv,
                                                       Value<AccScalar, S, C> &acc) {
  const int m = m_base + E;
...
FPSAN_DEVICE inline void wmma_16x16x128_accumulate_row_casted(const Acc *acast, int m_base, int idx,
                                                              int k, Acc bv, Acc &acc) {
  const int m = m_base + E;
...
template <class AVec, class BVec, class CVec, Semantics S, Conversions C>
FPSAN_DEVICE Value<CVec, S, C> wmma_16x16x128_dataflow(Value<AVec, S, C> a, Value<BVec, S, C> b,
                                                       Value<CVec, S, C> c) {
  using DFrag = Value<CVec, S, C>;
  using AccScalar = typename DFrag::element_type;
  using Acc = Value<AccScalar, S, C>;
  ...
  if constexpr (has_multiplicative_field_casts(S)) {
    ...
    for (int k = 0; k < 128; ++k) {
      const int idx = Wmma16x16x128Layout::ab_index(k);
      const Acc bv = cast<AccScalar>(wave_shfl(b.get(idx), Wmma16x16x128Layout::ab_lane(n, k)));
...
FPSAN_DEVICE Value<float, S, C> wmma_sub_gather_widen(const int (&pw)[16], int s, int srclane) {
...
FPSAN_DEVICE Value<float, S, C> wmma_f8f6f4_gather_widen(const int (&pw)[16], int s, int srclane) {
...
template <int E, int WA, Semantics S, Conversions C>
FPSAN_DEVICE inline void wmma_16x16x128_sub_accumulate_row(const int (&aw)[16], int m_base,
                                                           int slot, int k, Value<float, S, C> bv,
                                                           Value<float, S, C> &acc) {
  const int m = m_base + E;
  auto av = wmma_sub_gather_widen<WA, S, C>(aw, slot, Wmma16x16x128Layout::ab_lane(m, k));
  acc = acc + av * bv;
}

template <int WA, int WB, Semantics S, Conversions C>
FPSAN_DEVICE Value<v8f_native, S, C>
wmma_16x16x128_sub_dataflow(const int (&aw)[16], const int (&bw)[16], Value<v8f_native, S, C> c) {
  using Acc = Value<float, S, C>;
  const int lane = wave_lane();
  const int n = Wmma16x16x128Layout::cd_n(lane);
  const int m_base = 8 * (lane >> 4);
  ...
  for (int k = 0; k < 128; ++k) {
    ...
    auto bv = wmma_sub_gather_widen<WB, S, C>(bw, slot, Wmma16x16x128Layout::ab_lane(n, k));
    wmma_16x16x128_sub_accumulate_row<0, WA, S, C>(aw, m_base, slot, k, bv, acc0);
    wmma_16x16x128_sub_accumulate_row<1, WA, S, C>(aw, m_base, slot, k, bv, acc1);
    wmma_16x16x128_sub_accumulate_row<2, WA, S, C>(aw, m_base, slot, k, bv, acc2);
    wmma_16x16x128_sub_accumulate_row<3, WA, S, C>(aw, m_base, slot, k, bv, acc3);
    wmma_16x16x128_sub_accumulate_row<4, WA, S, C>(aw, m_base, slot, k, bv, acc4);
    wmma_16x16x128_sub_accumulate_row<5, WA, S, C>(aw, m_base, slot, k, bv, acc5);
    wmma_16x16x128_sub_accumulate_row<6, WA, S, C>(aw, m_base, slot, k, bv, acc6);
    wmma_16x16x128_sub_accumulate_row<7, WA, S, C>(aw, m_base, slot, k, bv, acc7);
  }
...
template <int FMT, Semantics S, Conversions C>
FPSAN_DEVICE inline Value<float, S, C> wmma_16x16x128_mixed_gather(const int (&w)[16],
                                                                   int row_or_col, int k) {
  const int slot = (FMT <= 1) ? Wmma16x16x128Layout::ab_index(k) : wmma_mix_sub_slot(k);
  const int src_lane =
      (FMT <= 1) ? Wmma16x16x128Layout::ab_lane(row_or_col, k) : wmma_mix_sub_lane(row_or_col, k);
  return wmma_f8f6f4_gather_widen<FMT, S, C>(w, slot, src_lane);
}

template <int E, int AFMT, Semantics S, Conversions C>
FPSAN_DEVICE inline void wmma_16x16x128_mixed_accumulate_row(const int (&aw)[16], int m_base, int k,
                                                             Value<float, S, C> bv,
                                                             Value<float, S, C> &acc) {
  const int m = m_base + E;
  auto av = wmma_16x16x128_mixed_gather<AFMT, S, C>(aw, m, k);
  acc = acc + av * bv;
}

template <int AFMT, int BFMT, Semantics S, Conversions C>
FPSAN_DEVICE Value<v8f_native, S, C>
wmma_16x16x128_mixed_dataflow(const int (&aw)[16], const int (&bw)[16], Value<v8f_native, S, C> c) {
  ...
  Acc acc7 = c.get(7);
  for (int k = 0; k < 128; ++k) {
    auto bv = wmma_16x16x128_mixed_gather<BFMT, S, C>(bw, n, k);
    wmma_16x16x128_mixed_accumulate_row<0, AFMT, S, C>(aw, m_base, k, bv, acc0);
    wmma_16x16x128_mixed_accumulate_row<1, AFMT, S, C>(aw, m_base, k, bv, acc1);
    wmma_16x16x128_mixed_accumulate_row<2, AFMT, S, C>(aw, m_base, k, bv, acc2);
    wmma_16x16x128_mixed_accumulate_row<3, AFMT, S, C>(aw, m_base, k, bv, acc3);
    wmma_16x16x128_mixed_accumulate_row<4, AFMT, S, C>(aw, m_base, k, bv, acc4);
    wmma_16x16x128_mixed_accumulate_row<5, AFMT, S, C>(aw, m_base, k, bv, acc5);
    wmma_16x16x128_mixed_accumulate_row<6, AFMT, S, C>(aw, m_base, k, bv, acc6);
    wmma_16x16x128_mixed_accumulate_row<7, AFMT, S, C>(aw, m_base, k, bv, acc7);
  }
  Value<v8f_native, S, C> d{};
  d.set(0, acc0);
  d.set(1, acc1);
...
FPSAN_HOST_DEVICE constexpr int wmma_sub_scale_byte(int k) { return 2 * (k >> 6) + ((k >> 2) & 1); }
...
FPSAN_HOST_DEVICE constexpr int wmma_mix_scale_byte(int k) { return k >> 5; }
...
template <int ScaleFmt> FPSAN_HOST_DEVICE inline float wmma_scale_byte_to_float(unsigned byte) {
  static_assert(ScaleFmt == 0 || ScaleFmt == 2, "WMMA scale format: 0 = E8M0FNU, 2 = E4M3FN");
...
template <class Acc>
FPSAN_DEVICE inline Acc wmma_select_scale4(int byte, const Acc &s0, const Acc &s1, const Acc &s2,
                                           const Acc &s3) {
  switch (byte) {
  case 0:
    return s0;
...
template <class Acc>
FPSAN_DEVICE inline Acc wmma_select_scale8(int byte, const Acc &s0, const Acc &s1, const Acc &s2,
                                           const Acc &s3, const Acc &s4, const Acc &s5,
                                           const Acc &s6, const Acc &s7) {
  switch (byte) {
  case 0:
    return s0;
  case 1:
...
template <class Acc> struct WmmaScale4 {
  Acc s0, s1, s2, s3;
  FPSAN_DEVICE inline Acc select(int byte) const {
    return wmma_select_scale4(byte, s0, s1, s2, s3);
  }
};
template <int ScaleFmt, class Acc, class Word>
FPSAN_DEVICE inline WmmaScale4<Acc> wmma_make_scale4(Word word) {
  return {wmma_scale_value_at_byte<ScaleFmt, Acc>(word, 0),
          wmma_scale_value_at_byte<ScaleFmt, Acc>(word, 1),
          wmma_scale_value_at_byte<ScaleFmt, Acc>(word, 2),
          wmma_scale_value_at_byte<ScaleFmt, Acc>(word, 3)};
}
template <class Acc> struct WmmaScale8 {
  Acc s0, s1, s2, s3, s4, s5, s6, s7;
  FPSAN_DEVICE inline Acc select(int byte) const {
    return wmma_select_scale8(byte, s0, s1, s2, s3, s4, s5, s6, s7);
  }
};
template <int ScaleFmt, class Acc, class Word>
FPSAN_DEVICE inline WmmaScale8<Acc> wmma_make_scale8(Word word) {
  return {wmma_scale_value_at_byte<ScaleFmt, Acc>(word, 0),
          wmma_scale_value_at_byte<ScaleFmt, Acc>(word, 1),
          wmma_scale_value_at_byte<ScaleFmt, Acc>(word, 2),
          wmma_scale_value_at_byte<ScaleFmt, Acc>(word, 3),
          wmma_scale_value_at_byte<ScaleFmt, Acc>(word, 4),
          wmma_scale_value_at_byte<ScaleFmt, Acc>(word, 5),
          wmma_scale_value_at_byte<ScaleFmt, Acc>(word, 6),
          wmma_scale_value_at_byte<ScaleFmt, Acc>(word, 7)};
}
...
template <int E, int WA, Semantics S, Conversions C, class Scale>
FPSAN_DEVICE inline void
wmma_16x16x128_sub_scaled_accumulate_row(const int (&aw)[16], int m_base, int slot, int k, int byte,
                                         Value<float, S, C> bv, Value<float, S, C> fb,
                                         const Scale &fa, Value<float, S, C> &acc) {
  const int m = m_base + E;
  auto av = wmma_sub_gather_widen<WA, S, C>(aw, slot, Wmma16x16x128Layout::ab_lane(m, k));
  const auto fav = fa.select(byte);
  acc = acc + av * bv * fav * fb;
}

template <int E0, int WA, int WB, Semantics S, Conversions C, int AScaleFmt>
FPSAN_DEVICE inline void wmma_16x16x128_sub_scaled_accumulate_group(
    const int (&aw)[16], const int (&bw)[16], int m_base, int n, int sa,
    const WmmaScale4<Value<float, S, C>> &fb_scale, const Value<v8f_native, S, C> &c,
    Value<v8f_native, S, C> &d) {
  using Acc = Value<float, S, C>;

  const int m0 = m_base + E0 + 0;
  const int m1 = m_base + E0 + 1;
  const int m2 = m_base + E0 + 2;
  const int m3 = m_base + E0 + 3;

  const unsigned wa0 = static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(m0 * 4, sa));
  const unsigned wa1 = static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(m1 * 4, sa));
  const unsigned wa2 = static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(m2 * 4, sa));
  const unsigned wa3 = static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(m3 * 4, sa));

  const auto fa0 = wmma_make_scale4<AScaleFmt, Acc>(wa0);
  const auto fa1 = wmma_make_scale4<AScaleFmt, Acc>(wa1);
  const auto fa2 = wmma_make_scale4<AScaleFmt, Acc>(wa2);
  const auto fa3 = wmma_make_scale4<AScaleFmt, Acc>(wa3);
  ...
  for (int k = 0; k < 128; ++k) {
    ...
    const Acc fb = fb_scale.select(byte);
    wmma_16x16x128_sub_scaled_accumulate_row<E0 + 0, WA, S, C>(aw, m_base, slot, k, byte, bv, fb,
                                                               fa0, acc0);
    wmma_16x16x128_sub_scaled_accumulate_row<E0 + 1, WA, S, C>(aw, m_base, slot, k, byte, bv, fb,
                                                               fa1, acc1);
    wmma_16x16x128_sub_scaled_accumulate_row<E0 + 2, WA, S, C>(aw, m_base, slot, k, byte, bv, fb,
                                                               fa2, acc2);
    wmma_16x16x128_sub_scaled_accumulate_row<E0 + 3, WA, S, C>(aw, m_base, slot, k, byte, bv, fb,
                                                               fa3, acc3);
  }
  d.set(E0 + 0, acc0);
  d.set(E0 + 1, acc1);
...
FPSAN_DEVICE Value<v8f_native, S, C>
wmma_16x16x128_sub_scaled_dataflow(const int (&aw)[16], const int (&bw)[16],
                                   Value<v8f_native, S, C> c, int sa, int sb) {
  using Acc = Value<float, S, C>;
  const int lane = wave_lane();
  const int n = Wmma16x16x128Layout::cd_n(lane);
  const int m_base = 8 * (lane >> 4);
  const unsigned wb = static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(n * 4, sb));
  const auto fb = wmma_make_scale4<BScaleFmt, Acc>(wb);
  Value<v8f_native, S, C> d{};
  wmma_16x16x128_sub_scaled_accumulate_group<0, WA, WB, S, C, AScaleFmt>(aw, bw, m_base, n, sa, fb,
                                                                         c, d);
  wmma_16x16x128_sub_scaled_accumulate_group<4, WA, WB, S, C, AScaleFmt>(aw, bw, m_base, n, sa, fb,
                                                                         c, d);
  return d;
}
...
template <int E0, int WA, int WB, Semantics S, Conversions C, int AScaleFmt>
FPSAN_DEVICE inline void wmma_16x16x128_sub_scaled16_accumulate_group(
    const int (&aw)[16], const int (&bw)[16], int m_base, int n, int sa_lo, int sa_hi,
    const WmmaScale8<Value<float, S, C>> &fb_scale, const Value<v8f_native, S, C> &c,
    Value<v8f_native, S, C> &d) {
  using Acc = Value<float, S, C>;
  ...
  const unsigned long long wa0 =
      (static_cast<unsigned long long>(
           static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(m0 * 4, sa_lo))) |
       (static_cast<unsigned long long>(
            static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(m0 * 4, sa_hi)))
        << 32));
  const unsigned long long wa1 =
      (static_cast<unsigned long long>(
           static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(m1 * 4, sa_lo))) |
       (static_cast<unsigned long long>(
            static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(m1 * 4, sa_hi)))
        << 32));
  const unsigned long long wa2 =
      (static_cast<unsigned long long>(
           static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(m2 * 4, sa_lo))) |
       (static_cast<unsigned long long>(
            static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(m2 * 4, sa_hi)))
        << 32));
  const unsigned long long wa3 =
      (static_cast<unsigned long long>(
           static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(m3 * 4, sa_lo))) |
       (static_cast<unsigned long long>(
            static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(m3 * 4, sa_hi)))
        << 32));

  const auto fa0 = wmma_make_scale8<AScaleFmt, Acc>(wa0);
  const auto fa1 = wmma_make_scale8<AScaleFmt, Acc>(wa1);
  const auto fa2 = wmma_make_scale8<AScaleFmt, Acc>(wa2);
  const auto fa3 = wmma_make_scale8<AScaleFmt, Acc>(wa3);
...
template <int WA, int WB, Semantics S, Conversions C, int AScaleFmt = 0, int BScaleFmt = 0>
FPSAN_DEVICE Value<v8f_native, S, C>
wmma_16x16x128_sub_scaled16_dataflow(const int (&aw)[16], const int (&bw)[16],
                                     Value<v8f_native, S, C> c, long long sa, long long sb) {
  using Acc = Value<float, S, C>;
  const int lane = wave_lane();
  const int n = Wmma16x16x128Layout::cd_n(lane);
  ...
  const unsigned long long wb =
      (static_cast<unsigned long long>(
       ...
       (static_cast<unsigned long long>(
        ...
        << 32));
  const auto fb = wmma_make_scale8<BScaleFmt, Acc>(wb);
  Value<v8f_native, S, C> d{};
  wmma_16x16x128_sub_scaled16_accumulate_group<0, WA, WB, S, C, AScaleFmt>(aw, bw, m_base, n, sa_lo,
                                                                           sa_hi, fb, c, d);
  wmma_16x16x128_sub_scaled16_accumulate_group<4, WA, WB, S, C, AScaleFmt>(aw, bw, m_base, n, sa_lo,
                                                                           sa_hi, fb, c, d);
  return d;
}
...
template <int E, int AFMT, Semantics S, Conversions C, class Scale>
FPSAN_DEVICE inline void
wmma_16x16x128_mixed_scaled_accumulate_row(const int (&aw)[16], int m_base, int k, int byte,
                                           Value<float, S, C> bv, Value<float, S, C> fb,
                                           const Scale &fa, Value<float, S, C> &acc) {
  const int m = m_base + E;
  auto av = wmma_16x16x128_mixed_gather<AFMT, S, C>(aw, m, k);
...
template <int E0, int AFMT, int BFMT, Semantics S, Conversions C, int AScaleFmt>
FPSAN_DEVICE inline void wmma_16x16x128_mixed_scaled_accumulate_group(
    const int (&aw)[16], const int (&bw)[16], int m_base, int n, int sa,
    const WmmaScale4<Value<float, S, C>> &fb_scale, const Value<v8f_native, S, C> &c,
    Value<v8f_native, S, C> &d) {
  using Acc = Value<float, S, C>;
  ...
  for (int k = 0; k < 128; ++k) {
    ...
    const Acc fb = fb_scale.select(byte);
    wmma_16x16x128_mixed_scaled_accumulate_row<E0 + 0, AFMT, S, C>(aw, m_base, k, byte, bv, fb, fa0,
                                                                   acc0);
    wmma_16x16x128_mixed_scaled_accumulate_row<E0 + 1, AFMT, S, C>(aw, m_base, k, byte, bv, fb, fa1,
                                                                   acc1);
    wmma_16x16x128_mixed_scaled_accumulate_row<E0 + 2, AFMT, S, C>(aw, m_base, k, byte, bv, fb, fa2,
                                                                   acc2);
    wmma_16x16x128_mixed_scaled_accumulate_row<E0 + 3, AFMT, S, C>(aw, m_base, k, byte, bv, fb, fa3,
                                                                   acc3);
  }
  d.set(E0 + 0, acc0);
  d.set(E0 + 1, acc1);
...
template <int AFMT, int BFMT, Semantics S, Conversions C, int AScaleFmt = 0, int BScaleFmt = 0>
FPSAN_DEVICE Value<v8f_native, S, C>
wmma_16x16x128_mixed_scaled_dataflow(const int (&aw)[16], const int (&bw)[16],
                                     Value<v8f_native, S, C> c, int sa, int sb) {
  static_assert((AFMT <= 1) != (BFMT <= 1),
  ...
  const auto fb = wmma_make_scale4<BScaleFmt, Acc>(wb);
  Value<v8f_native, S, C> d{};
  wmma_16x16x128_mixed_scaled_accumulate_group<0, AFMT, BFMT, S, C, AScaleFmt>(aw, bw, m_base, n,
                                                                               sa, fb, c, d);
  wmma_16x16x128_mixed_scaled_accumulate_group<4, AFMT, BFMT, S, C, AScaleFmt>(aw, bw, m_base, n,
                                                                               sa, fb, c, d);
  return d;
}
...
// gathered from lane m / lane n with a pair of 32-bit ds_bpermutes.
template <int E0, int AFMT, int BFMT, Semantics S, Conversions C, int AScaleFmt>
FPSAN_DEVICE inline void wmma_16x16x128_mixed_scaled16_accumulate_group(
    const int (&aw)[16], const int (&bw)[16], int m_base, int n, int sa_lo, int sa_hi,
    const WmmaScale8<Value<float, S, C>> &fb_scale, const Value<v8f_native, S, C> &c,
    Value<v8f_native, S, C> &d) {
  using Acc = Value<float, S, C>;
...
template <int AFMT, int BFMT, Semantics S, Conversions C, int AScaleFmt = 0, int BScaleFmt = 0>
FPSAN_DEVICE Value<v8f_native, S, C>
wmma_16x16x128_mixed_scaled16_dataflow(const int (&aw)[16], const int (&bw)[16],
                                       Value<v8f_native, S, C> c, long long sa, long long sb) {
  static_assert((AFMT <= 1) != (BFMT <= 1),
  ...
  const auto fb = wmma_make_scale8<BScaleFmt, Acc>(wb);
  Value<v8f_native, S, C> d{};
  wmma_16x16x128_mixed_scaled16_accumulate_group<0, AFMT, BFMT, S, C, AScaleFmt>(
      aw, bw, m_base, n, sa_lo, sa_hi, fb, c, d);
  wmma_16x16x128_mixed_scaled16_accumulate_group<4, AFMT, BFMT, S, C, AScaleFmt>(
      aw, bw, m_base, n, sa_lo, sa_hi, fb, c, d);
  return d;
}
...
struct Wmma32x16x128F4Layout {
  ...
  FPSAN_DEVICE static int b_lane(int n, int k) { return n + 16 * ((k >> 2) & 1); }
  FPSAN_DEVICE static int b_slot(int k) {
    const int reg = ((k >> 1) & 1) + 2 * ((k >> 3) & 1) + 4 * ((k >> 4) & 1) + 8 * ((k >> 5) & 1) +
                    16 * ((k >> 6) & 1);
    return 2 * reg + (k & 1);
  }
  FPSAN_DEVICE static int cd_m(int lane, int reg) { return reg + 16 * (lane >> 4); }
  FPSAN_DEVICE static int cd_n(int lane) { return lane & 15; }
...
template <int E, Semantics S, Conversions C>
FPSAN_DEVICE inline void wmma_32x16x128_f4_accumulate_row(const int (&aw)[16], int m_base, int k,
                                                          Value<float, S, C> bv,
                                                          Value<float, S, C> &acc) {
  const int m = m_base + E;
  auto av = wmma_sub_gather_widen<4, S, C>(aw, Wmma32x16x128F4Layout::a_slot(m, k),
...
wmma_32x16x128_f4_dataflow(const int (&aw)[16], const int (&bw)[16], Value<v16f_native, S, C> c) {
  using Acc = Value<float, S, C>;
  const int lane = wave_lane();
  const int n = Wmma32x16x128F4Layout::cd_n(lane);
  const int m_base = 16 * (lane >> 4);
  Acc acc0 = c.get(0);
  Acc acc1 = c.get(1);
  Acc acc2 = c.get(2);
  Acc acc3 = c.get(3);
  ...
  Acc acc15 = c.get(15);
  for (int k = 0; k < 128; ++k) {
    auto bv = wmma_sub_gather_widen<4, S, C>(bw, Wmma32x16x128F4Layout::b_slot(k),
                                             Wmma32x16x128F4Layout::b_lane(n, k));
    wmma_32x16x128_f4_accumulate_row<0, S, C>(aw, m_base, k, bv, acc0);
    wmma_32x16x128_f4_accumulate_row<1, S, C>(aw, m_base, k, bv, acc1);
    wmma_32x16x128_f4_accumulate_row<2, S, C>(aw, m_base, k, bv, acc2);
    wmma_32x16x128_f4_accumulate_row<3, S, C>(aw, m_base, k, bv, acc3);
    wmma_32x16x128_f4_accumulate_row<4, S, C>(aw, m_base, k, bv, acc4);
    wmma_32x16x128_f4_accumulate_row<5, S, C>(aw, m_base, k, bv, acc5);
    wmma_32x16x128_f4_accumulate_row<6, S, C>(aw, m_base, k, bv, acc6);
    wmma_32x16x128_f4_accumulate_row<7, S, C>(aw, m_base, k, bv, acc7);
    wmma_32x16x128_f4_accumulate_row<8, S, C>(aw, m_base, k, bv, acc8);
    wmma_32x16x128_f4_accumulate_row<9, S, C>(aw, m_base, k, bv, acc9);
    wmma_32x16x128_f4_accumulate_row<10, S, C>(aw, m_base, k, bv, acc10);
    wmma_32x16x128_f4_accumulate_row<11, S, C>(aw, m_base, k, bv, acc11);
    wmma_32x16x128_f4_accumulate_row<12, S, C>(aw, m_base, k, bv, acc12);
    wmma_32x16x128_f4_accumulate_row<13, S, C>(aw, m_base, k, bv, acc13);
    wmma_32x16x128_f4_accumulate_row<14, S, C>(aw, m_base, k, bv, acc14);
    wmma_32x16x128_f4_accumulate_row<15, S, C>(aw, m_base, k, bv, acc15);
  }
...
template <int E, Semantics S, Conversions C>
FPSAN_DEVICE inline void wmma_32x16x128_f4_scaled_accumulate_row(
    const int (&aw)[16], int m_base, int k, int byte, Value<float, S, C> bv, Value<float, S, C> fb,
    const WmmaScale4<Value<float, S, C>> &fa, Value<float, S, C> &acc) {
  const int m = m_base + E;
  auto av = wmma_sub_gather_widen<4, S, C>(aw, Wmma32x16x128F4Layout::a_slot(m, k),
                                           Wmma32x16x128F4Layout::a_lane(m, k));
  const auto fav = fa.select(byte);
...
template <int E0, Semantics S, Conversions C, int AScaleFmt>
FPSAN_DEVICE inline void wmma_32x16x128_f4_scaled_accumulate_group(
    const int (&aw)[16], const int (&bw)[16], int m_base, int n, int sa,
    const WmmaScale4<Value<float, S, C>> &fb_scale, const Value<v16f_native, S, C> &c,
    Value<v16f_native, S, C> &d) {
  using Acc = Value<float, S, C>;
  ...
  const unsigned wa0 = static_cast<unsigned>(
      __builtin_amdgcn_ds_bpermute(Wmma32x16x128F4Layout::scale_lane_a(m0) * 4, sa));
  const unsigned wa1 = static_cast<unsigned>(
      __builtin_amdgcn_ds_bpermute(Wmma32x16x128F4Layout::scale_lane_a(m1) * 4, sa));
  const unsigned wa2 = static_cast<unsigned>(
      __builtin_amdgcn_ds_bpermute(Wmma32x16x128F4Layout::scale_lane_a(m2) * 4, sa));
  const unsigned wa3 = static_cast<unsigned>(
      __builtin_amdgcn_ds_bpermute(Wmma32x16x128F4Layout::scale_lane_a(m3) * 4, sa));
  ...
  for (int k = 0; k < 128; ++k) {
    ...
    const int byte = wmma_sub_scale_byte(k);
    const Acc fb = fb_scale.select(byte);
    wmma_32x16x128_f4_scaled_accumulate_row<E0 + 0, S, C>(aw, m_base, k, byte, bv, fb, fa0, acc0);
    wmma_32x16x128_f4_scaled_accumulate_row<E0 + 1, S, C>(aw, m_base, k, byte, bv, fb, fa1, acc1);
    wmma_32x16x128_f4_scaled_accumulate_row<E0 + 2, S, C>(aw, m_base, k, byte, bv, fb, fa2, acc2);
    wmma_32x16x128_f4_scaled_accumulate_row<E0 + 3, S, C>(aw, m_base, k, byte, bv, fb, fa3, acc3);
  }
  d.set(E0 + 0, acc0);
  d.set(E0 + 1, acc1);
  d.set(E0 + 2, acc2);
...
FPSAN_DEVICE Value<v16f_native, S, C>
wmma_32x16x128_f4_scaled_dataflow(const int (&aw)[16], const int (&bw)[16],
                                  Value<v16f_native, S, C> c, int sa, int sb) {
  using Acc = Value<float, S, C>;
  const int lane = wave_lane();
  const int n = Wmma32x16x128F4Layout::cd_n(lane);
  const int m_base = 16 * (lane >> 4);
  ...
  const auto fb = wmma_make_scale4<BScaleFmt, Acc>(wb);
  Value<v16f_native, S, C> d{};
  wmma_32x16x128_f4_scaled_accumulate_group<0, S, C, AScaleFmt>(aw, bw, m_base, n, sa, fb, c, d);
  wmma_32x16x128_f4_scaled_accumulate_group<4, S, C, AScaleFmt>(aw, bw, m_base, n, sa, fb, c, d);
  wmma_32x16x128_f4_scaled_accumulate_group<8, S, C, AScaleFmt>(aw, bw, m_base, n, sa, fb, c, d);
  wmma_32x16x128_f4_scaled_accumulate_group<12, S, C, AScaleFmt>(aw, bw, m_base, n, sa, fb, c, d);
  return d;
}

template <int E, Semantics S, Conversions C>
FPSAN_DEVICE inline void wmma_32x16x128_f4_scaled16_accumulate_row(
    const int (&aw)[16], int m_base, int k, int byte, Value<float, S, C> bv, Value<float, S, C> fb,
    const WmmaScale8<Value<float, S, C>> &fa, Value<float, S, C> &acc) {
  const int m = m_base + E;
  auto av = wmma_sub_gather_widen<4, S, C>(aw, Wmma32x16x128F4Layout::a_slot(m, k),
                                           Wmma32x16x128F4Layout::a_lane(m, k));
  const auto fav = fa.select(byte);
...
template <int E0, Semantics S, Conversions C, int AScaleFmt>
FPSAN_DEVICE inline void wmma_32x16x128_f4_scaled16_accumulate_group(
    const int (&aw)[16], const int (&bw)[16], int m_base, int n, int sa_lo, int sa_hi,
    const WmmaScale8<Value<float, S, C>> &fb_scale, const Value<v16f_native, S, C> &c,
    Value<v16f_native, S, C> &d) {
  using Acc = Value<float, S, C>;
  ...
  const int la0 = Wmma32x16x128F4Layout::scale_lane_a(m0) * 4;
  const int la1 = Wmma32x16x128F4Layout::scale_lane_a(m1) * 4;
  const int la2 = Wmma32x16x128F4Layout::scale_lane_a(m2) * 4;
  const int la3 = Wmma32x16x128F4Layout::scale_lane_a(m3) * 4;

  const unsigned long long wa0 =
      (static_cast<unsigned long long>(
           static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(la0, sa_lo))) |
       (static_cast<unsigned long long>(
            static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(la0, sa_hi)))
        << 32));
  const unsigned long long wa1 =
      (static_cast<unsigned long long>(
           static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(la1, sa_lo))) |
       (static_cast<unsigned long long>(
            static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(la1, sa_hi)))
        << 32));
  const unsigned long long wa2 =
      (static_cast<unsigned long long>(
           static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(la2, sa_lo))) |
       (static_cast<unsigned long long>(
            static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(la2, sa_hi)))
        << 32));
  const unsigned long long wa3 =
      (static_cast<unsigned long long>(
           static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(la3, sa_lo))) |
       (static_cast<unsigned long long>(
            static_cast<unsigned>(__builtin_amdgcn_ds_bpermute(la3, sa_hi)))
        << 32));
  ...
  for (int k = 0; k < 128; ++k) {
    ...
    const int byte = wmma_sub_scale16_byte(k);
    const Acc fb = fb_scale.select(byte);
    wmma_32x16x128_f4_scaled16_accumulate_row<E0 + 0, S, C>(aw, m_base, k, byte, bv, fb, fa0, acc0);
    wmma_32x16x128_f4_scaled16_accumulate_row<E0 + 1, S, C>(aw, m_base, k, byte, bv, fb, fa1, acc1);
    wmma_32x16x128_f4_scaled16_accumulate_row<E0 + 2, S, C>(aw, m_base, k, byte, bv, fb, fa2, acc2);
    wmma_32x16x128_f4_scaled16_accumulate_row<E0 + 3, S, C>(aw, m_base, k, byte, bv, fb, fa3, acc3);
  }
  d.set(E0 + 0, acc0);
  d.set(E0 + 1, acc1);
  d.set(E0 + 2, acc2);
...
template <Semantics S, Conversions C, int AScaleFmt = 0, int BScaleFmt = 0>
FPSAN_DEVICE Value<v16f_native, S, C>
wmma_32x16x128_f4_scaled16_dataflow(const int (&aw)[16], const int (&bw)[16],
                                    Value<v16f_native, S, C> c, long long sa, long long sb) {
  using Acc = Value<float, S, C>;
  const int lane = wave_lane();
  const int n = Wmma32x16x128F4Layout::cd_n(lane);
  const int sa_lo = static_cast<int>(sa & 0xffffffff);
  ...
  Value<v16f_native, S, C> d{};
  const int m_base = 16 * (lane >> 4);
  wmma_32x16x128_f4_scaled16_accumulate_group<0, S, C, AScaleFmt>(aw, bw, m_base, n, sa_lo, sa_hi,
                                                                  fb, c, d);
  wmma_32x16x128_f4_scaled16_accumulate_group<4, S, C, AScaleFmt>(aw, bw, m_base, n, sa_lo, sa_hi,
                                                                  fb, c, d);
  wmma_32x16x128_f4_scaled16_accumulate_group<8, S, C, AScaleFmt>(aw, bw, m_base, n, sa_lo, sa_hi,
                                                                  fb, c, d);
  wmma_32x16x128_f4_scaled16_accumulate_group<12, S, C, AScaleFmt>(aw, bw, m_base, n, sa_lo, sa_hi,
                                                                   fb, c, d);
  return d;
}
...
FPSAN_DEVICE Value<v8f_native, S, C>
amdgcn_wmma_f32_16x16x16_f16_w32(Value<v16h_wmma_native, S, C> a, Value<v16h_wmma_native, S, C> b,
                                 Value<v8f_native, S, C> c) {
  if constexpr (S == Semantics::Native) {
    v8f_native d =
        __builtin_amdgcn_wmma_f32_16x16x16_f16_w32(a.to_float(), b.to_float(), c.to_float());
...
amdgcn_wmma_f32_16x16x16_bf16_w32(Value<v16bf_wmma_native, S, C> a,
                                  Value<v16bf_wmma_native, S, C> b, Value<v8f_native, S, C> c) {
  if constexpr (S == Semantics::Native) {
...
template <bool Opsel = false, Semantics S = Semantics::Native,
          Conversions C = Conversions::Explicit>
FPSAN_DEVICE Value<v16h_wmma_native, S, C>
amdgcn_wmma_f16_16x16x16_f16_w32(Value<v16h_wmma_native, S, C> a, Value<v16h_wmma_native, S, C> b,
                                 Value<v16h_wmma_native, S, C> c) {
  if constexpr (S == Semantics::Native) {
    v16h_wmma_native d =
        __builtin_amdgcn_wmma_f16_16x16x16_f16_w32(a.to_float(), b.to_float(), c.to_float(), Opsel);
...
amdgcn_wmma_f16_16x16x16_f16_tied_w32(Value<v16h_wmma_native, S, C> a,
                                      ...
                                      Value<v16h_wmma_native, S, C> c) {
  if constexpr (S == Semantics::Native) {
    v16h_wmma_native d = __builtin_amdgcn_wmma_f16_16x16x16_f16_tied_w32(a.to_float(), b.to_float(),
                                                                         c.to_float(), Opsel);
    return Value<v16h_wmma_native, S, C>(d);
  } else {
...
amdgcn_wmma_bf16_16x16x16_bf16_w32(Value<v16bf_wmma_native, S, C> a,
  ...
  if constexpr (S == Semantics::Native) {
    ...
    const v16i16_wmma_native di = __builtin_amdgcn_wmma_bf16_16x16x16_bf16_w32(ai, bi, ci, Opsel);
...
amdgcn_wmma_f32_16x16x16_f16_w64(Value<v16h_wmma_native, S, C> a, Value<v16h_wmma_native, S, C> b,
                                 Value<v4f_native, S, C> c) {
  if constexpr (S == Semantics::Native) {
    v4f_native d =
        __builtin_amdgcn_wmma_f32_16x16x16_f16_w64(a.to_float(), b.to_float(), c.to_float());
...
amdgcn_wmma_f32_16x16x16_bf16_w64(Value<v16bf_wmma_native, S, C> a,
                                  Value<v16bf_wmma_native, S, C> b, Value<v4f_native, S, C> c) {
  if constexpr (S == Semantics::Native) {
    const v16i16_wmma_native ai = __builtin_bit_cast(v16i16_wmma_native, a.to_float());
    const v16i16_wmma_native bi = __builtin_bit_cast(v16i16_wmma_native, b.to_float());
...
amdgcn_wmma_f16_16x16x16_f16_w64(Value<v16h_wmma_native, S, C> a, Value<v16h_wmma_native, S, C> b,
                                 Value<v8h_native, S, C> c) {
  if constexpr (S == Semantics::Native) {
    v8h_native d =
        __builtin_amdgcn_wmma_f16_16x16x16_f16_w64(a.to_float(), b.to_float(), c.to_float(), Opsel);
...
amdgcn_wmma_f16_16x16x16_f16_tied_w64(Value<v16h_wmma_native, S, C> a,
                                      Value<v16h_wmma_native, S, C> b, Value<v8h_native, S, C> c) {
  if constexpr (S == Semantics::Native) {
    v8h_native d = __builtin_amdgcn_wmma_f16_16x16x16_f16_tied_w64(a.to_float(), b.to_float(),
                                                                   c.to_float(), Opsel);
    return Value<v8h_native, S, C>(d);
  } else {
    return detail::wmma_gfx11_half_16x16x16_dataflow<Opsel, true>(a, b, c);
...
amdgcn_wmma_bf16_16x16x16_bf16_w64(Value<v16bf_wmma_native, S, C> a,
                                   Value<v16bf_wmma_native, S, C> b, Value<v8bf_native, S, C> c) {
  if constexpr (S == Semantics::Native) {
    const v16i16_wmma_native ai = __builtin_bit_cast(v16i16_wmma_native, a.to_float());
    const v16i16_wmma_native bi = __builtin_bit_cast(v16i16_wmma_native, b.to_float());
    const v8i16_wmma_native ci = __builtin_bit_cast(v8i16_wmma_native, c.to_float());
    const v8i16_wmma_native di = __builtin_amdgcn_wmma_bf16_16x16x16_bf16_w64(ai, bi, ci, Opsel);
  ...
  FPSAN_DEVICE Value<v8f_native, S, C> NAME(Value<AVec_, S, C> a, Value<BVec_, S, C> b,            \
    ...
    if constexpr (S == Semantics::Native) {                                                        \
      v8f_native d = BUILTIN(__builtin_bit_cast(v2i32_native, a.to_float()),                       \
                             __builtin_bit_cast(v2i32_native, b.to_float()), c.to_float());        \
      return Value<v8f_native, S, C>(d);                                                           \
    } else {                                                                                       \
      return detail::wmma_16x16x16_dataflow(a, b, c);                                              \
    }                                                                                              \
...
FPSAN_DEFINE_WMMA_16X16X16(amdgcn_wmma_f32_16x16x16_f16_w32, v8h_native, v8h_native, v8f_native,
                           __builtin_amdgcn_wmma_f32_16x16x16_f16_w32_gfx12)
FPSAN_DEFINE_WMMA_16X16X16(amdgcn_wmma_f16_16x16x16_f16_w32, v8h_native, v8h_native, v8h_native,
                           __builtin_amdgcn_wmma_f16_16x16x16_f16_w32_gfx12)
FPSAN_DEFINE_WMMA_16X16X16(amdgcn_wmma_f32_16x16x16_bf16_w32, v8bf_native, v8bf_native, v8f_native,
                           __builtin_amdgcn_wmma_f32_16x16x16_bf16_w32_gfx12)
FPSAN_DEFINE_WMMA_16X16X16(amdgcn_wmma_bf16_16x16x16_bf16_w32, v8bf_native, v8bf_native,
                           v8bf_native, __builtin_amdgcn_wmma_bf16_16x16x16_bf16_w32_gfx12)

FPSAN_DEFINE_WMMA_16X16X16_FP8(amdgcn_wmma_f32_16x16x16_fp8_fp8_w32, v8e4m3_native, v8e4m3_native,
                               __builtin_amdgcn_wmma_f32_16x16x16_fp8_fp8_w32_gfx12)
FPSAN_DEFINE_WMMA_16X16X16_FP8(amdgcn_wmma_f32_16x16x16_fp8_bf8_w32, v8e4m3_native, v8e5m2_native,
                               __builtin_amdgcn_wmma_f32_16x16x16_fp8_bf8_w32_gfx12)
FPSAN_DEFINE_WMMA_16X16X16_FP8(amdgcn_wmma_f32_16x16x16_bf8_fp8_w32, v8e5m2_native, v8e4m3_native,
                               __builtin_amdgcn_wmma_f32_16x16x16_bf8_fp8_w32_gfx12)
FPSAN_DEFINE_WMMA_16X16X16_FP8(amdgcn_wmma_f32_16x16x16_bf8_bf8_w32, v8e5m2_native, v8e5m2_native,
...
FPSAN_DEVICE Value<v4f_native, S, C> amdgcn_wmma_f32_16x16x16_f16_w64(Value<v4h_native, S, C> a,
  ...
  if constexpr (S == Semantics::Native) {
    v4f_native d =
        __builtin_amdgcn_wmma_f32_16x16x16_f16_w64_gfx12(a.to_float(), b.to_float(), c.to_float());
    return Value<v4f_native, S, C>(d);
  } else {
    return detail::wmma_16x16x16_dataflow(a, b, c);
  }
...
template <Semantics S, Conversions C>
FPSAN_DEVICE Value<v4f_native, S, C> amdgcn_wmma_f32_16x16x16_bf16_w64(Value<v4bf_native, S, C> a,
                                                                       Value<v4bf_native, S, C> b,
                                                                       Value<v4f_native, S, C> c) {
  if constexpr (S == Semantics::Native) {
...
FPSAN_DEVICE Value<v4h_native, S, C> amdgcn_wmma_f16_16x16x16_f16_w64(Value<v4h_native, S, C> a,
  ...
  if constexpr (S == Semantics::Native) {
    v4h_native d =
        __builtin_amdgcn_wmma_f16_16x16x16_f16_w64_gfx12(a.to_float(), b.to_float(), c.to_float());
...
amdgcn_wmma_bf16_16x16x16_bf16_w64(Value<v4bf_native, S, C> a, Value<v4bf_native, S, C> b,
                                   Value<v4bf_native, S, C> c) {
  if constexpr (S == Semantics::Native) {
    const v4i16_native ai = __builtin_bit_cast(v4i16_native, a.to_float());
    const v4i16_native bi = __builtin_bit_cast(v4i16_native, b.to_float());
...
template <Semantics S, Conversions C>
FPSAN_DEVICE Value<v4f_native, S, C>
amdgcn_wmma_f32_16x16x16_fp8_fp8_w64(Value<v4e4m3_native, S, C> a, Value<v4e4m3_native, S, C> b,
                                     Value<v4f_native, S, C> c) {
  if constexpr (S == Semantics::Native) {
    v4f_native d = __builtin_amdgcn_wmma_f32_16x16x16_fp8_fp8_w64_gfx12(
        __builtin_bit_cast(int, a.to_float()), __builtin_bit_cast(int, b.to_float()), c.to_float());
    return Value<v4f_native, S, C>(d);
  } else {
    return detail::wmma_16x16x16_dataflow(a, b, c);
  }
...
template <Semantics S, Conversions C>
FPSAN_DEVICE Value<v4f_native, S, C>
amdgcn_wmma_f32_16x16x16_fp8_bf8_w64(Value<v4e4m3_native, S, C> a, Value<v4e5m2_native, S, C> b,
                                     Value<v4f_native, S, C> c) {
  if constexpr (S == Semantics::Native) {
...
template <Semantics S, Conversions C>
FPSAN_DEVICE Value<v4f_native, S, C>
amdgcn_wmma_f32_16x16x16_bf8_fp8_w64(Value<v4e5m2_native, S, C> a, Value<v4e4m3_native, S, C> b,
                                     Value<v4f_native, S, C> c) {
  if constexpr (S == Semantics::Native) {
...
template <Semantics S, Conversions C>
FPSAN_DEVICE Value<v4f_native, S, C>
amdgcn_wmma_f32_16x16x16_bf8_bf8_w64(Value<v4e5m2_native, S, C> a, Value<v4e5m2_native, S, C> b,
                                     Value<v4f_native, S, C> c) {
  if constexpr (S == Semantics::Native) {
...
FPSAN_DEVICE Value<v8f_native, S, C> amdgcn_wmma_f32_16x16x4_f32(Value<v2f_native, S, C> a,
  ...
  if constexpr (S == Semantics::Native) {
    v8f_native d =
        __builtin_amdgcn_wmma_f32_16x16x4_f32(false, a.to_float(), false, b.to_float(),
                                              static_cast<short>(Cmod), c.to_float(), false, false);
    return Value<v8f_native, S, C>(d);
  } else {
...
#define FPSAN_DEFINE_WMMA_16X16X32(NAME, AVec_, CVec_, BUILTIN)                                    \
  template <Semantics S, Conversions C, int Cmod = 0>                                              \
  FPSAN_DEVICE Value<CVec_, S, C> NAME(Value<AVec_, S, C> a, Value<AVec_, S, C> b,                 \
                                       Value<CVec_, S, C> c) {                                     \
    if constexpr (S == Semantics::Native) {                                                        \
      CVec_ d = BUILTIN(false, a.to_float(), false, b.to_float(), static_cast<short>(Cmod),        \
                        c.to_float(), false, false);                                               \
      return Value<CVec_, S, C>(d);                                                                \
    } else {                                                                                       \
...
FPSAN_DEFINE_WMMA_16X16X32(amdgcn_wmma_f32_16x16x32_f16, v16h_native, v8f_native,
                           __builtin_amdgcn_wmma_f32_16x16x32_f16)
FPSAN_DEFINE_WMMA_16X16X32(amdgcn_wmma_f16_16x16x32_f16, v16h_native, v8h_native,
                           __builtin_amdgcn_wmma_f16_16x16x32_f16)
FPSAN_DEFINE_WMMA_16X16X32(amdgcn_wmma_f32_16x16x32_bf16, v16bf_native, v8f_native,
                           __builtin_amdgcn_wmma_f32_16x16x32_bf16)
FPSAN_DEFINE_WMMA_16X16X32(amdgcn_wmma_bf16_16x16x32_bf16, v16bf_native, v8bf_native,
...
template <Semantics S, Conversions C, int Cmod = 0>
FPSAN_DEVICE Value<v8bf_native, S, C> amdgcn_wmma_bf16f32_16x16x32_bf16(Value<v16bf_native, S, C> a,
                                                                        Value<v16bf_native, S, C> b,
                                                                        Value<v8f_native, S, C> c) {
  if constexpr (S == Semantics::Native) {
    v8bf_native d = __builtin_amdgcn_wmma_bf16f32_16x16x32_bf16(
        false, a.to_float(), false, b.to_float(), static_cast<short>(Cmod), c.to_float(), false,
        false);
    return Value<v8bf_native, S, C>(d);
  } else {
    return detail::wmma_16x16x32_dataflow_cdiff<v8bf_native, v16bf_native, v16bf_native,
                                                v8f_native>(a, b, detail::wmma_apply_cmod<Cmod>(c));
  }
}
  ...
  FPSAN_DEVICE Value<CVec_, S, C> NAME(Value<AVec_, S, C> a, Value<BVec_, S, C> b,                 \
    ...
    if constexpr (S == Semantics::Native) {                                                        \
      CVec_ d = BUILTIN(__builtin_bit_cast(v8i32_native, a.to_float()),                            \
                        __builtin_bit_cast(v8i32_native, b.to_float()), static_cast<short>(Cmod),  \
                        c.to_float(), false, false);                                               \
      return Value<CVec_, S, C>(d);                                                                \
    } else {                                                                                       \
...
FPSAN_DEFINE_WMMA_16X16X64_FP8(amdgcn_wmma_f32_16x16x64_fp8_fp8, v32e4m3_native, v32e4m3_native,
                               v8f_native, __builtin_amdgcn_wmma_f32_16x16x64_fp8_fp8)
FPSAN_DEFINE_WMMA_16X16X64_FP8(amdgcn_wmma_f32_16x16x64_fp8_bf8, v32e4m3_native, v32e5m2_native,
                               v8f_native, __builtin_amdgcn_wmma_f32_16x16x64_fp8_bf8)
FPSAN_DEFINE_WMMA_16X16X64_FP8(amdgcn_wmma_f32_16x16x64_bf8_fp8, v32e5m2_native, v32e4m3_native,
                               v8f_native, __builtin_amdgcn_wmma_f32_16x16x64_bf8_fp8)
FPSAN_DEFINE_WMMA_16X16X64_FP8(amdgcn_wmma_f32_16x16x64_bf8_bf8, v32e5m2_native, v32e5m2_native,
                               v8f_native, __builtin_amdgcn_wmma_f32_16x16x64_bf8_bf8)

FPSAN_DEFINE_WMMA_16X16X64_FP8(amdgcn_wmma_f16_16x16x64_fp8_fp8, v32e4m3_native, v32e4m3_native,
                               v8h_native, __builtin_amdgcn_wmma_f16_16x16x64_fp8_fp8)
FPSAN_DEFINE_WMMA_16X16X64_FP8(amdgcn_wmma_f16_16x16x64_fp8_bf8, v32e4m3_native, v32e5m2_native,
                               v8h_native, __builtin_amdgcn_wmma_f16_16x16x64_fp8_bf8)
FPSAN_DEFINE_WMMA_16X16X64_FP8(amdgcn_wmma_f16_16x16x64_bf8_fp8, v32e5m2_native, v32e4m3_native,
                               v8h_native, __builtin_amdgcn_wmma_f16_16x16x64_bf8_fp8)
FPSAN_DEFINE_WMMA_16X16X64_FP8(amdgcn_wmma_f16_16x16x64_bf8_bf8, v32e5m2_native, v32e5m2_native,
                               v8h_native, __builtin_amdgcn_wmma_f16_16x16x64_bf8_bf8)
  ...
  FPSAN_DEVICE Value<CVec_, S, C> NAME(Value<AVec_, S, C> a, Value<BVec_, S, C> b,                 \
    ...
    if constexpr (S == Semantics::Native) {                                                        \
      CVec_ d = BUILTIN(__builtin_bit_cast(v16i32_native, a.to_float()),                           \
                        __builtin_bit_cast(v16i32_native, b.to_float()), static_cast<short>(Cmod), \
                        c.to_float(), false, false);                                               \
      return Value<CVec_, S, C>(d);                                                                \
    } else {                                                                                       \
...
FPSAN_DEFINE_WMMA_16X16X128_FP8(amdgcn_wmma_f32_16x16x128_fp8_fp8, v64e4m3_native, v64e4m3_native,
                                v8f_native, __builtin_amdgcn_wmma_f32_16x16x128_fp8_fp8)
FPSAN_DEFINE_WMMA_16X16X128_FP8(amdgcn_wmma_f32_16x16x128_fp8_bf8, v64e4m3_native, v64e5m2_native,
                                v8f_native, __builtin_amdgcn_wmma_f32_16x16x128_fp8_bf8)
FPSAN_DEFINE_WMMA_16X16X128_FP8(amdgcn_wmma_f32_16x16x128_bf8_fp8, v64e5m2_native, v64e4m3_native,
                                v8f_native, __builtin_amdgcn_wmma_f32_16x16x128_bf8_fp8)
FPSAN_DEFINE_WMMA_16X16X128_FP8(amdgcn_wmma_f32_16x16x128_bf8_bf8, v64e5m2_native, v64e5m2_native,
                                v8f_native, __builtin_amdgcn_wmma_f32_16x16x128_bf8_bf8)

FPSAN_DEFINE_WMMA_16X16X128_FP8(amdgcn_wmma_f16_16x16x128_fp8_fp8, v64e4m3_native, v64e4m3_native,
                                v8h_native, __builtin_amdgcn_wmma_f16_16x16x128_fp8_fp8)
FPSAN_DEFINE_WMMA_16X16X128_FP8(amdgcn_wmma_f16_16x16x128_fp8_bf8, v64e4m3_native, v64e5m2_native,
                                v8h_native, __builtin_amdgcn_wmma_f16_16x16x128_fp8_bf8)
FPSAN_DEFINE_WMMA_16X16X128_FP8(amdgcn_wmma_f16_16x16x128_bf8_fp8, v64e5m2_native, v64e4m3_native,
                                v8h_native, __builtin_amdgcn_wmma_f16_16x16x128_bf8_fp8)
FPSAN_DEFINE_WMMA_16X16X128_FP8(amdgcn_wmma_f16_16x16x128_bf8_bf8, v64e5m2_native, v64e5m2_native,
                                v8h_native, __builtin_amdgcn_wmma_f16_16x16x128_bf8_bf8)
...
template <int AFMT, int BFMT, Semantics S = Semantics::Native,
          Conversions C = Conversions::Explicit, int Cmod = 0>
FPSAN_DEVICE Value<v8f_native, S, C>
amdgcn_wmma_f32_16x16x128_f8f6f4_sub(v16i32_native a, v16i32_native b, Value<v8f_native, S, C> c) {
...
template <int AFMT, int BFMT, Semantics S = Semantics::Native,
          Conversions C = Conversions::Explicit, int Cmod = 0>
FPSAN_DEVICE Value<v8f_native, S, C>
amdgcn_wmma_f32_16x16x128_f8f6f4_mixed(v16i32_native a, v16i32_native b,
                                       Value<v8f_native, S, C> c) {
  static_assert((AFMT <= 1) != (BFMT <= 1),
  ...
  static_assert(AFMT >= 0 && AFMT <= 4 && BFMT >= 0 && BFMT <= 4, "f8f6f4 format must be 0-4");
  if constexpr (S == Semantics::Native) {
    v8f_native d = __builtin_amdgcn_wmma_f32_16x16x128_f8f6f4(
        AFMT, a, BFMT, b, static_cast<short>(Cmod), c.to_float());
    return Value<v8f_native, S, C>(d);
...
template <int AFMT, int BFMT, Semantics S = Semantics::Native,
          Conversions C = Conversions::Explicit, int Cmod = 0, int AScaleFmt = 0, int BScaleFmt = 0>
FPSAN_DEVICE Value<v8f_native, S, C>
amdgcn_wmma_scale_f32_16x16x128_f8f6f4_sub(v16i32_native a, v16i32_native b,
                                           Value<v8f_native, S, C> c, int sa, int sb) {
  static_assert(AFMT >= 2 && AFMT <= 4 && BFMT >= 2 && BFMT <= 4,
  ...
  if constexpr (S == Semantics::Native) {
    v8f_native d = __builtin_amdgcn_wmma_scale_f32_16x16x128_f8f6f4(
        AFMT, a, BFMT, b, static_cast<short>(Cmod), c.to_float(), 0, AScaleFmt, sa, 0, BScaleFmt,
        sb, false, false);
    return Value<v8f_native, S, C>(d);
  } else {
    int aw[16], bw[16];
    ...
    return detail::wmma_16x16x128_sub_scaled_dataflow<
        detail::f8f6f4_width(AFMT), detail::f8f6f4_width(BFMT), S, C, AScaleFmt, BScaleFmt>(
...
template <int AFMT, int BFMT, Semantics S = Semantics::Native,
          Conversions C = Conversions::Explicit, int Cmod = 0, int AScaleFmt = 0, int BScaleFmt = 0>
FPSAN_DEVICE Value<v8f_native, S, C>
amdgcn_wmma_scale_f32_16x16x128_f8f6f4_mixed(v16i32_native a, v16i32_native b,
                                             Value<v8f_native, S, C> c, int sa, int sb) {
  static_assert((AFMT <= 1) != (BFMT <= 1),
  ...
  if constexpr (S == Semantics::Native) {
    ...
    return detail::wmma_16x16x128_mixed_scaled_dataflow<AFMT, BFMT, S, C, AScaleFmt, BScaleFmt>(
        aw, bw, detail::wmma_apply_cmod<Cmod>(c), sa, sb);
  }
}
...
template <int AFMT, int BFMT, Semantics S = Semantics::Native,
          Conversions C = Conversions::Explicit, int Cmod = 0, int AScaleFmt = 0, int BScaleFmt = 0>
FPSAN_DEVICE Value<v8f_native, S, C>
amdgcn_wmma_scale16_f32_16x16x128_f8f6f4_sub(v16i32_native a, v16i32_native b,
                                             Value<v8f_native, S, C> c, long sa, long sb) {
  static_assert(AFMT >= 2 && AFMT <= 4 && BFMT >= 2 && BFMT <= 4,
                "the _sub wrapper is for fp6/bf6/fp4 formats (2-4); mixing an 8-bit operand "
                "with a sub-byte one needs a dedicated WMMA mix model (not yet grounded).");
  if constexpr (S == Semantics::Native) {
    ...
    return detail::wmma_16x16x128_sub_scaled16_dataflow<
        detail::f8f6f4_width(AFMT), detail::f8f6f4_width(BFMT), S, C, AScaleFmt, BScaleFmt>(
        aw, bw, detail::wmma_apply_cmod<Cmod>(c), static_cast<long long>(sa),
...
template <int AFMT, int BFMT, Semantics S = Semantics::Native,
          Conversions C = Conversions::Explicit, int Cmod = 0, int AScaleFmt = 0, int BScaleFmt = 0>
FPSAN_DEVICE Value<v8f_native, S, C>
amdgcn_wmma_scale16_f32_16x16x128_f8f6f4_mixed(v16i32_native a, v16i32_native b,
                                               Value<v8f_native, S, C> c, long sa, long sb) {
  static_assert((AFMT <= 1) != (BFMT <= 1),
  ...
  if constexpr (S == Semantics::Native) {
    ...
    return detail::wmma_16x16x128_mixed_scaled16_dataflow<AFMT, BFMT, S, C, AScaleFmt, BScaleFmt>(
        aw, bw, detail::wmma_apply_cmod<Cmod>(c), static_cast<long long>(sa),
        static_cast<long long>(sb));
  }
}
...
FPSAN_DEVICE Value<v16f_native, S, C> amdgcn_wmma_f32_32x16x128_f4(v16i32_native a, v8i32_native b,
                                                                   Value<v16f_native, S, C> c) {
  if constexpr (S == Semantics::Native) {
    v16f_native d =
        __builtin_amdgcn_wmma_f32_32x16x128_f4(a, b, static_cast<short>(Cmod), c.to_float());
    ...
    for (int w = 0; w < 16; ++w)
      bw[w] = (w < 8) ? b[w] : 0;
    return detail::wmma_32x16x128_f4_dataflow<S, C>(aw, bw, detail::wmma_apply_cmod<Cmod>(c));
  }
}
...
template <Semantics S = Semantics::Native, Conversions C = Conversions::Explicit, int Cmod = 0,
          int AScaleFmt = 0, int BScaleFmt = 0>
FPSAN_DEVICE Value<v16f_native, S, C>
amdgcn_wmma_scale_f32_32x16x128_f4(v16i32_native a, v8i32_native b, Value<v16f_native, S, C> c,
                                   int sa, int sb) {
  if constexpr (S == Semantics::Native) {
    v16f_native d = __builtin_amdgcn_wmma_scale_f32_32x16x128_f4(a, b, static_cast<short>(Cmod),
                                                                 c.to_float(), 0, AScaleFmt, sa, 0,
                                                                 BScaleFmt, sb, false, false);
    return Value<v16f_native, S, C>(d);
  } else {
    int aw[16], bw[16];
    for (int w = 0; w < 16; ++w)
...
template <Semantics S = Semantics::Native, Conversions C = Conversions::Explicit, int Cmod = 0,
          int AScaleFmt = 0, int BScaleFmt = 0>
FPSAN_DEVICE Value<v16f_native, S, C>
amdgcn_wmma_scale16_f32_32x16x128_f4(v16i32_native a, v8i32_native b, Value<v16f_native, S, C> c,
                                     long sa, long sb) {
  if constexpr (S == Semantics::Native) {
    v16f_native d = __builtin_amdgcn_wmma_scale16_f32_32x16x128_f4(a, b, static_cast<short>(Cmod),
                                                                   c.to_float(), 0, AScaleFmt, sa,
                                                                   0, BScaleFmt, sb, false, false);
    return Value<v16f_native, S, C>(d);
  } else {
    int aw[16], bw[16];
    for (int w = 0; w < 16; ++w)
