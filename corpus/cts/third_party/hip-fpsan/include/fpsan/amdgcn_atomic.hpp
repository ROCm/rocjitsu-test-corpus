...
template <bool IsMin, class Float, class UInt> FPSAN_DEVICE Float atomic_fmm(Float *p, Float v) {
...
FPSAN_DEVICE Value<VEC, S, C> atomic_pk_add(Value<VEC, S, C> *addr, Value<VEC, S, C> v) {
  ...
  static_assert(sizeof(V) == sizeof(unsigned), "packed atomic expects a 32-bit (2x16-bit) Value");
  auto *p = reinterpret_cast<unsigned *>(addr);
  unsigned old = *p;
  for (;;) {
    const V oldV = V::from_storage_bits(__builtin_bit_cast(typename V::bits_type, old));
