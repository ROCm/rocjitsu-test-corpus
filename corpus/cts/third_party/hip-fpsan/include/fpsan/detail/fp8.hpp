...
FPSAN_HOST_DEVICE inline float narrow_to_f32(std::uint32_t src, const FpFormat &f) {
  ...
  if (src_exp == exp_mask) {
    ...
    return sgn * std::ldexp(static_cast<float>(src_mantissa), 1 - exp_bias - f.mantissa_bits);
  }
  return sgn * std::ldexp(static_cast<float>(src_mantissa + (1u << f.mantissa_bits)),
                          (static_cast<int>(src_exp) >> exp_shift) - exp_bias - f.mantissa_bits);
}
...
struct fp8_e4m3 {
  std::uint8_t bits;
  FPSAN_HOST_DEVICE constexpr fp8_e4m3() : bits(0) {}
  FPSAN_HOST_DEVICE constexpr explicit fp8_e4m3(std::uint8_t b) : bits(b) {}
  FPSAN_HOST_DEVICE explicit fp8_e4m3(float v)
      : bits(static_cast<std::uint8_t>(detail::f32_to_narrow(v, detail::kFp8E4M3))) {}
  FPSAN_HOST_DEVICE operator float() const { return detail::narrow_to_f32(bits, detail::kFp8E4M3); }
  ...
  FPSAN_HOST_DEVICE friend bool operator==(fp8_e4m3 a, fp8_e4m3 b) { return a.bits == b.bits; }
  FPSAN_HOST_DEVICE friend bool operator!=(fp8_e4m3 a, fp8_e4m3 b) { return !(a == b); }
};

struct fp8_e5m2 {
  std::uint8_t bits;
  FPSAN_HOST_DEVICE constexpr fp8_e5m2() : bits(0) {}
  FPSAN_HOST_DEVICE constexpr explicit fp8_e5m2(std::uint8_t b) : bits(b) {}
  FPSAN_HOST_DEVICE explicit fp8_e5m2(float v)
      : bits(static_cast<std::uint8_t>(detail::f32_to_narrow(v, detail::kFp8E5M2))) {}
  FPSAN_HOST_DEVICE operator float() const { return detail::narrow_to_f32(bits, detail::kFp8E5M2); }
  FPSAN_HOST_DEVICE friend bool operator<(fp8_e5m2 a, fp8_e5m2 b) {
    return static_cast<float>(a) < static_cast<float>(b);
  }
  FPSAN_HOST_DEVICE friend bool operator==(fp8_e5m2 a, fp8_e5m2 b) { return a.bits == b.bits; }
  FPSAN_HOST_DEVICE friend bool operator!=(fp8_e5m2 a, fp8_e5m2 b) { return !(a == b); }
};

#define FPSAN_DEFINE_FP8_SCALAR(NAME, FORMAT)                                                      \
  struct NAME {                                                                                    \
    std::uint8_t bits;                                                                             \
    FPSAN_HOST_DEVICE constexpr NAME() : bits(0) {}                                                \
    FPSAN_HOST_DEVICE constexpr explicit NAME(std::uint8_t b) : bits(b) {}                         \
    FPSAN_HOST_DEVICE explicit NAME(float v)                                                       \
        : bits(static_cast<std::uint8_t>(detail::f32_to_narrow(v, FORMAT))) {}                     \
    FPSAN_HOST_DEVICE operator float() const { return detail::narrow_to_f32(bits, FORMAT); }       \
    FPSAN_HOST_DEVICE friend bool operator<(NAME a, NAME b) {                                      \
      return static_cast<float>(a) < static_cast<float>(b);                                        \
    }                                                                                              \
    FPSAN_HOST_DEVICE friend bool operator==(NAME a, NAME b) { return a.bits == b.bits; }          \
    FPSAN_HOST_DEVICE friend bool operator!=(NAME a, NAME b) { return !(a == b); }                 \
  };
