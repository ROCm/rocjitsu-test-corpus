...
template <class T> FPSAN_HOST_DEVICE constexpr MixConfig make_mix_config() {
...
template <class Bits> FPSAN_HOST_DEVICE constexpr Bits bits_embed(const MixConfig &c, Bits raw) {
...
template <class Bits> FPSAN_HOST_DEVICE constexpr Bits ring_neg(const MixConfig &c, Bits a) {
  using L = bits_lane_t<Bits>;
...
FPSAN_HOST_DEVICE constexpr Bits ring_div(const MixConfig &c, Bits a, Bits b) {
  if constexpr (!is_clang_vector_v<Bits>) {
    ...
    Bits out{};
    for (unsigned i = 0; i < sizeof(Bits) / sizeof(L); ++i)
      out[i] = static_cast<L>(payload_div(c, static_cast<u64>(a[i]), static_cast<u64>(b[i])));
    return out;
  }
}
