...
FPSAN_HOST_DEVICE constexpr u64 payload_tagged_unary(const MixConfig &c, u64 p, UnaryOpId op) {
...
// argument-order-sensitive tag, the only structure an opaque function can
...
// tag = (sum_i rotl(payload_i, i)) XOR nameHash, all mod 2^w -- exactly
...
template <class... P>
FPSAN_HOST_DEVICE constexpr u64 payload_extern_tagged(const MixConfig &c, u64 name_hash,
                                                      P... operands) {
  u64 acc = 0;
  unsigned i = 0;
