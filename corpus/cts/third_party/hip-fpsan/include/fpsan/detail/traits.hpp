...
template <unsigned Bytes> struct uint_by_size;
template <> struct uint_by_size<1> {
  using type = std::uint8_t;
};
template <> struct uint_by_size<2> {
  using type = std::uint16_t;
};
template <> struct uint_by_size<4> {
  using type = std::uint32_t;
};
template <> struct uint_by_size<8> {
...
template <class T> struct fp_traits {
  static_assert(always_false<T>, "fpsan: unsupported floating-point type. Supported: float, "
                                 "double, _Float16, __bf16, and the fpsan FP8 scalar "
                                 "types where available.");
};

#define FPSAN_DEFINE_FP_TRAITS_FORMAT(T, MANT, EXP, BIAS, CAST_TAG, HAS_INFINITY, HAS_NAN,         \
                                      NAN_AS_NEG_ZERO)                                             \
  template <> struct fp_traits<T> {                                                                \
    using value_type = T;                                                                          \
...
template <class T, class = void> inline constexpr bool is_supported_float_v = false;
template <class T>
inline constexpr bool is_supported_float_v<T, std::void_t<typename fp_traits<T>::bits_type>> = true;
...
template <class T, class = void> inline constexpr bool is_clang_vector_v = false;
template <class T>
inline constexpr bool is_clang_vector_v<T, std::void_t<decltype(std::declval<T &>()[0])>> = true;
...
template <class LaneBits, unsigned long Bytes> struct uint_vector {
...
template <class B, bool = is_clang_vector_v<B>> struct bits_lane {
  using type = B;
};
template <class B> struct bits_lane<B, true> {
...
template <class B> struct signed_bits<B, true> {
  using type = uint_vector_t<std::make_signed_t<vector_element_t<B>>, sizeof(B)>;
};
template <class B> using signed_bits_t = typename signed_bits<B>::type;

template <class T, bool = is_clang_vector_v<T>> struct value_traits {
  // Scalar case.
...
template <class T> struct value_traits<T, true> {
