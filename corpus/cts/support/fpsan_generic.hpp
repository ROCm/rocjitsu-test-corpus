...
namespace fpsan_generic
{
    ...
    struct FPFormat
    {
        ...
        // amount.
        uint32_t oneBits() const
        {
            return static_cast<uint32_t>(bias) << mantBits;
        }
    };
    ...
    public:
        FPSanFloat(const FPFormat& fmt, uint32_t payload)
            : fmt_(&fmt)
            , payload_(payload & mix_config_for(fmt).fullMask)
        {
        }
        ...
        uint32_t raw_bits() const
        {
            return unmix_int_to_float_bits(cfg(), payload_);
        }
        ...
        FPSanFloat operator-() const
        {
            return wrap(0u - payload_);
        }

        FPSanFloat exp() const
        {
            return wrap(fpsan_exp_payload(cfg(), payload_));
        }
        FPSanFloat exp2() const
        {
            return wrap(fpsan_exp2_payload(cfg(), payload_));
        }
    ...
    private:
        const MixConfig& cfg() const
        {
            return mix_config_for(*fmt_);
        }
