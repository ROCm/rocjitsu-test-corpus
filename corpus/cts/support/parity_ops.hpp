...
inline constexpr std::uint32_t kRingLaws = kAssocAdd | kAssocMul | kDistrib | kCommAdd | kCommMul;
...
FPSAN_HOST_DEVICE inline ParitySample parity_compute(FT a, FT b, FT c) {
  ...
  r.storage_div = bzero ? std::uint64_t{0} : static_cast<std::uint64_t>((A / B).to_storage_bits());
