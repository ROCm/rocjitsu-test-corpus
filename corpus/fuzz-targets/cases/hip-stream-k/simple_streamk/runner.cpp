...
#include <streamk/simple_streamk.hpp>
...
#include <streamk/utils/print.hpp>
...
int main(int argc, char* argv[]) {
  using input_t = float32_t;
  using output_t = float32_t;
  ...
  simple_streamk<input_t, output_t, compute_t>(
  ...
  double elapsed = simple_streamk<input_t, output_t, compute_t>(
      m, n, k, a.data().get(), b.data().get(), c.data().get(), d.data().get(),
      m, n, m, m, alpha, beta, grid, cli.num_runs);
  statistics<output_t>(m, n, k, elapsed, "simple_streamk");
}