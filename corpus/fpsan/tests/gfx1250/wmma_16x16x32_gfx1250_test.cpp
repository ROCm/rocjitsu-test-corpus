...
TEST(WmmaF32F16_32, LayoutMatchesHardware)
{
    run_layout_matches_hardware<WmmaF32F16_32>();
}
...
TEST(WmmaF16F16_32, LayoutMatchesHardware)
{
    run_layout_matches_hardware<WmmaF16F16_32>();
}
...
TEST(WmmaF32BF16_32, LayoutMatchesHardware)
{
    run_layout_matches_hardware<WmmaF32BF16_32>();
}
...
TEST(WmmaBF16BF16_32, LayoutMatchesHardware)
{
    run_layout_matches_hardware<WmmaBF16BF16_32>();
}
...
void run_modifier_f16()
{
    ...
    for(int mm = 0; mm < M; ++mm)
        for(int nn = 0; nn < N; ++nn)
        {
            ...
            for(int k = 0; k < K; ++k)
                acc = acc
                      + fpsan::cast<float>(VA(m.A[mm * K + k]))
                            * fpsan::cast<float>(VA(m.B[k * N + nn]));
    ...
    for(int i = 0; i < M * N; ++i)
    {
        ...
        EXPECT_EQ(gotP[i], pref[i])
            << "FPSan mod mismatch Cmod=" << Cmod << " at " << (i / N) << "," << (i % N);
    }
    (void)hipFree(dA);
    (void)hipFree(dB);
    (void)hipFree(dC);
...
__device__ void load_frags_bf16f32(const __bf16*                A,
                                   const __bf16*                B,
                                   const float*                 C,
                                   int                          lane,
...
TEST(WmmaBF16F32_32, FpsanMatchesScalarReference)
{
    ...
    for(int mm = 0; mm < M; ++mm)
        for(int nn = 0; nn < N; ++nn)
        {
            ...
            for(int k = 0; k < K; ++k)
                acc = acc
                      + fpsan::cast<float>(VBF(A[mm * K + k]))
                            * fpsan::cast<float>(VBF(B[k * N + nn]));
