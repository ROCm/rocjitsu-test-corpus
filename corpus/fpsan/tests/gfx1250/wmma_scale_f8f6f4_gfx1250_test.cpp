...
static int ab_lane(int rc, int k)
{
    return rc + 16 * ((k >> 2) & 1);
}
...
__global__ void k_scale(const std::uint32_t* Apack,
...
TEST(WmmaScaleF8f6f4_128, LayoutMatchesHardware)
{
    ...
    for(int m = 0; m < M; ++m)
        for(int k = 0; k < K; ++k)
            pack_byte(
                &Apack[ab_lane(m, k) * 16], ab_slot(k), f32_to_narrow(A[m * K + k], kFp8E4M3));
    for(int n = 0; n < N; ++n)
        for(int k = 0; k < K; ++k)
            pack_byte(
                &Bpack[ab_lane(n, k) * 16], ab_slot(k), f32_to_narrow(B[k * N + n], kFp8E4M3));
...
static void make_scales(std::mt19937&          rng,
                        std::vector<int>&      eA,
                        std::vector<int>&      eB,
...
__global__ void k_sub_scale_float(const std::uint32_t* Apack,
                                  const std::uint32_t* Bpack,
                                  const float*         C,
                                  const unsigned*      SA,
                                  const unsigned*      SB,
...
static void run_sub_scale(const char* tag)
{
    ...
    for(int m = 0; m < M; ++m)
        for(int k = 0; k < K; ++k)
            pack_field(&Apack[ab_lane(m, k) * 16],
                       ab_slot(k),
                       f32_to_narrow(A[m * K + k], fmt_of(AFMT)),
                       aw);
    for(int n = 0; n < N; ++n)
        for(int k = 0; k < K; ++k)
            pack_field(&Bpack[ab_lane(n, k) * 16],
                       ab_slot(k),
                       f32_to_narrow(B[k * N + n], fmt_of(BFMT)),
                       bw);
...
__global__ void
    k_probe(const std::uint32_t* Apack, const std::uint32_t* Bpack, const unsigned* SA, float* D)
{
    const int     lane = threadIdx.x;
    v16i32_native a, b;
    for(int i = 0; i < 16; ++i)
...
template <int AFMT>
__global__ void k_probe2(const std::uint32_t* Apack,
                         const std::uint32_t* Bpack,
                         const unsigned*      SA,
                         const unsigned*      SB,
                         float*               D)
...
template <int AFMT>
static void run_probe(const char* tag)
{
    if(!have_device())
        GTEST_SKIP() << "no HIP device";
...
TEST(WmmaScaleF8f6f4_128, DISABLED_ProbeScaleLayoutFp4)
{
    run_probe<4>("fp4");
}
TEST(WmmaScaleF8f6f4_128, DISABLED_ProbeScaleLayoutFp6)
{
    run_probe<2>("fp6");
}
TEST(WmmaScaleF8f6f4_128, DISABLED_ProbeScaleLayoutFp8)
{
    run_probe<0>("fp8");
}
...
template <int AFMT>
__global__ void k_probe16(const std::uint32_t*      Apack,
                          const std::uint32_t*      Bpack,
...
template <int AFMT>
static void run_probe16(const char* tag)
{
    if(!have_device())
        GTEST_SKIP() << "no HIP device";
    const int                  w = width_of(AFMT);
...
TEST(WmmaScaleF8f6f4_128, DISABLED_ProbeScale16LayoutFp4)
{
    run_probe16<4>("fp4");
}
TEST(WmmaScaleF8f6f4_128, DISABLED_ProbeScale16LayoutFp6)
{
    run_probe16<2>("fp6");
}
TEST(WmmaScaleF8f6f4_128, DISABLED_ProbeScale16LayoutFp8)
{
    run_probe16<0>("fp8");
}
...
__global__ void k_sub_scale16_float(const std::uint32_t*      Apack,
                                    const std::uint32_t*      Bpack,
                                    const float*              C,
                                    const unsigned long long* SA,
                                    const unsigned long long* SB,
...
template <int AFMT, int BFMT>
static void stage_mixed(const std::vector<float>&   A,
    ...
    for(int m = 0; m < M; ++m)
        for(int k = 0; k < K; ++k)
            pack_field(&Apack[(aSub ? mix_lane(m, k) : ab_lane(m, k)) * 16],
                       aSub ? mix_slot(k) : ab_slot(k),
                       f32_to_narrow(A[m * K + k], fmt_of(AFMT)),
                       aw);
    for(int n = 0; n < N; ++n)
        for(int k = 0; k < K; ++k)
            pack_field(&Bpack[(bSub ? mix_lane(n, k) : ab_lane(n, k)) * 16],
                       bSub ? mix_slot(k) : ab_slot(k),
                       f32_to_narrow(B[k * N + n], fmt_of(BFMT)),
                       bw);
}

template <int AFMT, int BFMT, int ASFMT, int BSFMT>
__global__ void k_mix_scale_float(const std::uint32_t* Apack,
                                  const std::uint32_t* Bpack,
                                  const float*         C,
                                  const unsigned*      SA,
                                  const unsigned*      SB,
...
template <int AFMT, int BFMT>
__global__ void k_mix_scale16_float(const std::uint32_t*      Apack,
                                    const std::uint32_t*      Bpack,
                                    const float*              C,
                                    const unsigned long long* SA,
                                    const unsigned long long* SB,
