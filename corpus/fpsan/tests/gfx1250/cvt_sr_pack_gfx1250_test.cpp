...
template <class FP8>
void run_chain(void (*k)(float, float, float, float, int*))
{
    if(!have_device())
...
TEST(CvtSrPack, ChainFp8F32)
{
    run_chain<fpsan::fp8_e4m3>(k_chain_fp8_f32);
}
TEST(CvtSrPack, ChainBf8F32)
{
    run_chain<fpsan::fp8_e5m2>(k_chain_bf8_f32);
}
...
template <class FP8>
void run_chain_f16(void (*k)(_Float16, _Float16, _Float16, _Float16, int*))
{
    if(!have_device())
        GTEST_SKIP() << "no HIP device";
    const _Float16 v[4] = {static_cast<_Float16>(1.0f),
...
TEST(CvtSrPack, ChainFp8F16)
{
    run_chain_f16<fpsan::fp8_e4m3>(k_chain_fp8_f16);
}
TEST(CvtSrPack, ChainBf8F16)
{
    run_chain_f16<fpsan::fp8_e5m2>(k_chain_bf8_f16);
}
