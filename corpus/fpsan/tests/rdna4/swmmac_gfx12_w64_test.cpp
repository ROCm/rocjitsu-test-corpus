...
namespace
{
    ...
    int encode_idx_h_w64(int lane, const std::vector<int>& p0, const std::vector<int>& p1)
    {
        const int i     = lane & 15;
    ...
    int encode_idx_fp8_w64(int lane, const std::vector<int>& p0, const std::vector<int>& p1)
    {
        const int i     = lane & 15;
        const int block = lane >> 4;
        int       v     = 0;
        ...
        for(int m = 0; m < M; ++m)
            for(int n = 0; n < N; ++n)
            {
                ...
                for(int g = 0; g < G; ++g)
                {
                    ...
                    acc = acc
                          + fpsan::cast<CScalar>(VA(static_cast<AScalar>(d.A[m * K + k1])))
        ...
        for(int m = 0; m < M; ++m)
            for(int n = 0; n < N; ++n)
            {
                ...
                for(int g = 0; g < G; ++g)
                {
                    ...
                    acc = acc
                          + fpsan::cast<float>(VA(static_cast<AScalar>(d.A[m * K + k1])))
    ...
    struct SwF32F16
    {
        ...
        __device__ static Value<CVec, S, kCC>
            call(Value<AVec, S, kCC> a, Value<BVec, S, kCC> b, Value<CVec, S, kCC> c, int idx)
        {
    ...
    __global__ void k_swmmac_h(const float* A,
                               const float* B,
                               const float* C,
                               const int*   p0,
                               const int*   p1,
    ...
    template <class AScalar_, class BScalar_, class AVec_, class BVec_, class Wrap>
    struct SwFp8
    {
    ...
    using SwFp8Fp8 = SwFp8<fpsan::fp8_e4m3,
                           fpsan::fp8_e4m3,
                           fpsan::v4e4m3_native,
                           fpsan::v8e4m3_native,
                           Fp8Fp8Wrap>;
    using SwFp8Bf8 = SwFp8<fpsan::fp8_e4m3,
                           fpsan::fp8_e5m2,
                           fpsan::v4e4m3_native,
                           fpsan::v8e5m2_native,
                           Fp8Bf8Wrap>;
    using SwBf8Fp8 = SwFp8<fpsan::fp8_e5m2,
                           fpsan::fp8_e4m3,
                           fpsan::v4e5m2_native,
                           fpsan::v8e4m3_native,
                           Bf8Fp8Wrap>;
    using SwBf8Bf8 = SwFp8<fpsan::fp8_e5m2,
                           fpsan::fp8_e5m2,
                           fpsan::v4e5m2_native,
                           fpsan::v8e5m2_native,
                           Bf8Bf8Wrap>;
    ...
    template <class Traits, Semantics S, class Out>
    __global__ void k_swmmac_fp8(const float* A,
                                 const float* B,
                                 const float* C,
                                 const int*   p0,
                                 const int*   p1,
    ...
    void run_h_case(std::uint32_t seed)
    {
        if(!have_device())
    ...
    template <class Traits>
    void run_fp8_case(std::uint32_t seed)
    {
        if(!have_device())
            GTEST_SKIP() << "no HIP device";
        SwData             d = make_sw_data(seed, encode_idx_fp8_w64);
        std::vector<float> ref
            = sw_reference_fp8<typename Traits::AScalar, typename Traits::BScalar>(d);
...
TEST(SwmmacGfx12W64, F32_F16_LayoutAndFpsan)
{
    run_h_case<SwF32F16>(0x1000);
}
TEST(SwmmacGfx12W64, F32_BF16_LayoutAndFpsan)
{
    run_h_case<SwF32BF16>(0x2000);
}
TEST(SwmmacGfx12W64, F16_F16_LayoutAndFpsan)
{
    run_h_case<SwF16F16>(0x3000);
}
TEST(SwmmacGfx12W64, BF16_BF16_LayoutAndFpsan)
{
    run_h_case<SwBF16BF16>(0x4000);
}
TEST(SwmmacGfx12W64, F32_FP8_FP8_LayoutAndFpsan)
{
    run_fp8_case<SwFp8Fp8>(0x5000);
}
TEST(SwmmacGfx12W64, F32_FP8_BF8_LayoutAndFpsan)
{
    run_fp8_case<SwFp8Bf8>(0x6000);
}
TEST(SwmmacGfx12W64, F32_BF8_FP8_LayoutAndFpsan)
{
    run_fp8_case<SwBf8Fp8>(0x7000);
}
TEST(SwmmacGfx12W64, F32_BF8_BF8_LayoutAndFpsan)
{
    run_fp8_case<SwBf8Bf8>(0x8000);
}
