...
class DashboardPublishTest(unittest.TestCase):
    ...
    def test_corpus_provenance_is_preserved_and_checked(self) -> None:
        self._publish(expected_corpus_sha="E" * 40)
        run = publisher.load_json_document(self.data / "runs/github-123-attempt-1.json")
        details = {item["key"]: item["value"] for item in run["provenance"]["details"]}
        self.assertEqual(details["corpusCommitSha"], "e" * 40)
        self.assertEqual(details["corpusCommitTimestamp"], "2026-09-01T00:00:00Z")

    def test_rejects_invalid_corpus_provenance(self) -> None:
        for field, value, expected in (
            ("corpusCommitSha", "f" * 40, "does not match expected corpus SHA"),
            ("corpusCommitSha", "invalid", "40 hexadecimal digits"),
            ("corpusCommitSha", None, "corpus commit SHA"),
            ("corpusDirty", True, "clean corpus checkout"),
            ("corpusDirty", None, "clean corpus checkout"),
        ):
            with self.subTest(field=field, value=value):
                raw = self._raw()
                raw["provenance"][field] = value
                with self.assertRaisesRegex(publisher.PublishError, expected):
                    self._publish(raw, expected_corpus_sha="e" * 40)
        with self.assertRaisesRegex(publisher.PublishError, "expected corpus SHA"):
            self._publish(expected_corpus_sha="invalid")

    def test_historical_corpus_provenance_is_optional_without_expectation(self) -> None:
        raw = self._raw()
        for field in ("corpusCommitSha", "corpusCommitTimestamp", "corpusDirty"):
            del raw["provenance"][field]
        with self.assertRaisesRegex(publisher.PublishError, "clean corpus checkout"):
            self._publish(raw, expected_corpus_sha="e" * 40)
        self._publish(raw)

    def test_explicitly_dirty_corpus_is_rejected_without_expectation(self) -> None:
        raw = self._raw()
        raw["provenance"]["corpusDirty"] = True
        with self.assertRaisesRegex(publisher.PublishError, "clean corpus checkout"):
            self._publish(raw)
    ...
    def test_repeat_is_idempotent_and_keeps_index_timestamp(self) -> None:
        ...
        self.assertEqual(
            len(publisher.load_json_document(self.data / "index.json")["runs"]), 1
        )
    ...
    def test_new_run_merges_catalog_and_preserves_history(self) -> None:
        ...
        index = publisher.load_json_document(self.data / "index.json")
        self.assertEqual(
            [item["runId"] for item in index["runs"]],
            [
    ...
    def test_conflicting_definition_fails_before_writing_resources(self) -> None:
        ...
        with self.assertRaisesRegex(publisher.PublishError, "definition conflicts"):
            self._publish(self._raw([conflicting]), run_id="github-124-attempt-1")
        self.assertEqual((self.data / "index.json").read_bytes(), before)
        self.assertFalse((self.data / "runs" / "github-124-attempt-1.json").exists())
    ...
    def test_completed_result_cannot_borrow_a_catalog_problem(self) -> None:
        ...
        with self.assertRaisesRegex(
            publisher.PublishError, "completed test.*no problem"
        ):
    ...
    def test_requires_clean_matching_source_revision(self) -> None:
        ...
        with self.assertRaisesRegex(publisher.PublishError, "clean Rocjitsu checkout"):
            self._publish(dirty)
        with self.assertRaisesRegex(
            publisher.PublishError, "does not match expected SHA"
        ):
    ...
    def test_missing_indexed_run_is_rejected_before_index_update(self) -> None:
        ...
        with self.assertRaisesRegex(
            publisher.PublishError, "indexed dashboard run is missing"
        ):
