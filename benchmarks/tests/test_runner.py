...
""",
    ...
    def test_first_partial_run_publishes_and_failed_case_can_later_succeed(
        ...
        self.assertEqual(len(normalized["tests"]), 4)
        for actual, raw in zip(normalized["tests"], partial["tests"], strict=True):
            self.assertEqual(actual["status"], raw["status"])
            self.assertEqual(actual["problem"], raw["problem"])
