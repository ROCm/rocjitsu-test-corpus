...
def _definition(test: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "operation": _string(test.get("operation"), f"test {logical_id} operation"),
        "dataType": data_type,
...
def _catalog_definition(test: Mapping[str, Any]) -> dict[str, Any]:
    logical_id = _string(test.get("id", test.get("logicalTestId")), "catalog test ID")
...
def _details(
    provenance: Mapping[str, Any], source_ref: str | None
) -> list[dict[str, Any]]:
    fields = (
        ("sourceRef", "Source ref", source_ref),
        ("corpusCommitSha", "Corpus commit", provenance.get("corpusCommitSha")),
        (
            "corpusCommitTimestamp",
            "Corpus commit timestamp",
            provenance.get("corpusCommitTimestamp"),
        ),
...
def _validate_result(test: Mapping[str, Any], definition: Mapping[str, Any]) -> None:
    ...
    for field in ("suite", "name", "operation", "dataType", "execMode", "numThreads"):
        if observed[field] != definition[field]:
            raise PublishError(
                f"test {logical_id} conflicts with catalog field {field}"
            )
...
def normalize_run(
    ...
    *,
    ...
    if commit_order is not None and (
        isinstance(commit_order, bool)
        or not isinstance(commit_order, int)
        or commit_order < 0
    ):
    ...
    if "corpusDirty" in raw_provenance or expected_corpus_sha is not None:
        if raw_provenance.get("corpusDirty") is not False:
            raise PublishError("raw run must come from a clean corpus checkout")
    corpus_sha = raw_provenance.get("corpusCommitSha")
    if corpus_sha is not None or expected_corpus_sha is not None:
        corpus_sha = _string(corpus_sha, "corpus commit SHA")
        if not SHA.fullmatch(corpus_sha):
            raise PublishError("corpus commit SHA must contain 40 hexadecimal digits")
    if expected_corpus_sha is not None:
        expected_corpus_sha = _string(expected_corpus_sha, "expected corpus SHA")
        if not SHA.fullmatch(expected_corpus_sha):
            raise PublishError("expected corpus SHA must contain 40 hexadecimal digits")
        if corpus_sha.lower() != expected_corpus_sha.lower():
            raise PublishError(
                f"raw corpus commit SHA {corpus_sha} does not match "
                f"expected corpus SHA {expected_corpus_sha}"
            )
...
def _validate_dashboard_run(
    ...
    if run.get("branch") != "develop" or run.get("trigger") not in ("auto", "manual"):
        raise PublishError(
            f"dashboard run {expected_run_id} is not an official develop run"
        )
    ...
    for value in tests:
        ...
        if test_id in seen:
            raise PublishError(
                f"dashboard run {expected_run_id} has duplicate test {test_id}"
            )
        seen.add(test_id)
...
def _load_index(data_dir: Path) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
    ...
    if not catalog_bytes or hashlib.sha256(catalog_bytes).hexdigest()[
        :12
    ] != match.group(1):
...
def publish(
    ...
    *,
    ...
    index_changed = not matching or (
        index is not None and index["catalog"] != catalog_name
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--expected-corpus-sha")
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = publish(
            ...
            expected_corpus_sha=arguments.expected_corpus_sha,
        )
